...
// --- [신규] 1/3 Octave band bin index map (32 bands x 3 resolutions x 2 indices = 192 bytes, 내부 SRAM 상주) ---
...
// --- [신규] 31차 힐버트 FIR 필터 계수 정의 (.rodata Flash XIP 상주) ---
...
void CL_T2_DspEngine::processAccel(const float* p_inX, const float* p_inY, const float* p_inZ,
    ...
    for (int i = 0; i < T2_Def::Accel::Sensor::AXIS_MAX; i++) {
        ...
        // [신규] 적응형 2단계 메디안 필터링 (풀스케일 95% 초과만 적용, 그 외 바이패스)
        if (v_dsp.med_en) {
        ...
        // [신규] 힐버트 변환 진폭 포락선 추출 및 군지연 보정 (31차 FIR, 군지연 15)
