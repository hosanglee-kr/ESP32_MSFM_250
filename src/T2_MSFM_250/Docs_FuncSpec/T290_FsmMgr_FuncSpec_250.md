...
*   **부팅 시퀀스 흐름**:
    1.  [T220_CfgMgr](../T220_CfgMgr_250.hpp)을 가동하여 플래시 메모리에서 최종 저장된 JSON 설정을 복구 로드합니다. (Cold Boot 시 `esp_reset_reason()` 판정으로 `g_CrashDiag`을 강제 소거합니다.)
...
*   **기상 임계 및 복귀 트리거**: 딥슬립 설정 진입 및 Any-Motion 감지 시 INT2 핀 RISING 엣지를 통해 장비를 웨이크업시킵니다.
...
#### `void prepareForOta()` / `void resumeFromOtaFailure()`
*   **기능설명**: OTA 진입 시 하드웨어 인터럽트와 DMA 채널을 차단하여 캐시 미스 크래시를 방어하는 `prepareForOta()`와, OTA 플래시 라이트 실패 시 펌웨어 수집 스트림과 가공 파이프라인을 복원하고 인터럽트를 원복하는 `resumeFromOtaFailure()` 복구 인터페이스입니다.
