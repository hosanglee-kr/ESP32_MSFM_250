...
*   **기능설명**: 요구 시퀀스 길이(기본 16)와 차원수(예: 312)를 기반으로 SIMD 가속 정렬 오프셋을 맞추어 PSRAM 내에 평탄 버퍼(`_dataFlat`)를 할당 초기화합니다.
...
*   **기능설명**: 인가받은 오디오 타임스탬프 `audio_ts` 기준과 직전 수입된 진동 데이터의 타임스탬프 스큐를 비교 분석하여 허용치 이내일 경우, 최적 매핑된 진동 특징 벡터 데이터를 `out_feats`에 복사해 줍니다. 오디오 타임스탬프가 진동 스냅샷 사이(prev ~ curr)에 존재할 경우 선형 보간하여 정합하고, 그 외 범위를 벗어난 예외 상황에서는 최근의 최신 스냅샷 값을 그대로 유지하는 **ZOH(Zero Order Hold) 폴백** 방식을 수행합니다.
