/* ============================================================================
 * File: T245_FeatExtra_248.cpp
 * Summary: 멀티모달(진동/소음) 융합 특징량 추출 엔진 구현부 (v245 개정판)
 * ============================================================================ */
...
bool CL_T2_FeatureExtractor::init(const T2_Type::ST_Audio_Config_t& audioCfg) {
    ...
    // --- Audio Mel Filterbank (실제 샘플레이트 기반 동적 생성) ---
    reloadAudioMelFilter((float)audioCfg.sample_rate);

    // --- 완료 ---
    _isInitialized = true;
    ESP_LOGI(TAG, "Feature Engine v245 Initialized (Audio SR: %d Hz)", audioCfg.sample_rate);
    return true;
}
...
void CL_T2_FeatureExtractor::extractAccel(const float* p_inX, const float* p_inY, const float* p_inZ, uint32_t p_len, uint32_t p_sampleRate,
    ...
    for (int i = 0; i < T2_Def::Accel::Sensor::AXIS_MAX; i++) {
        ...
        // [Step 4] 진동 슬롯 MFCC 버퍼 선두 영역 채널 전달 (채널 ID: 0, 1, 2)
...
void CL_T2_FeatureExtractor::extractGyro(const float* p_inX, const float* p_inY, const float* p_inZ, uint32_t p_len, uint32_t p_sampleRate,
    ...
    for (int i = 0; i < T2_Def::Gyro::Sensor::AXIS_MAX; i++) {
        ...
        // [Step 1] 관성 통계 지표 추출 및 오프셋 정렬
        _computeStats(v_in[i], p_len, p_vibSlot.gyro.rms[i], p_vibSlot.gyro.kurt[i], p_vibSlot.gyro.crest[i], p_vibSlot.gyro.skew[i], p_vibSlot.gyro.std[i]);
        p_vibSlot.gyro.cal_off[i] = p_gyrCfg.offset[i];
        p_vibSlot.gyro.drift_est[i] = 0.0f;

        // [Step 2] FFT 수행 및 파워 변환
        ...
        // [Step 4] 자이로 전용 MFCC 영역에 주입 (채널 ID: 3, 4, 5)
        _computeMfcc(_powerIMU, v_bins, p_vibSlot.mfcc, 3 + i, false);
    }
}

/* ============================================================================
 * File: T245_FeatExtra_248.cpp (extractAudio 교정 완료 완전체 블록)
 * ============================================================================ */
...
void CL_T2_FeatureExtractor::extractAudio(const float* p_audL, const float* p_audR, uint32_t p_len, uint32_t p_sampleRate,
    ...
    // [교정 완비] 컴파일 스코프 에러 파괴를 위한 로컬 제어 변수 최상단 강제 통합 선언
    ...
    if (p_audL && hasLeft) {
        ...
        for (uint32_t i = 0; i < v_bins; i++) {
            float re = v_fftLeft[i * 2]; float im = v_fftLeft[i * 2 + 1];
        ...
        _computeMfcc(_powerAudio, v_bins, p_audSlot.mfcc, 6, true);
    }
    ...
    if (p_audR && hasRight) {
        ...
        for (uint32_t i = 0; i < v_bins; i++) {
            float re = v_fftRight[i * 2]; float im = v_fftRight[i * 2 + 1];
            _powerAudio[i] = (re * re + im * im) / v_len;
        }
    ...
    // 양방향 Coherence & IPD 지표 수립
    if (p_audL && p_audR && hasLeft && hasRight) {
        float v_cohSum = 0; float v_ipdSum = 0; uint32_t v_cohCount = 0;
        for (uint32_t i = 1; i < v_bins - 1; i++) {
            ...
            float C_re = L_re * R_re + L_im * R_im; float C_im = L_im * R_re - L_re * R_im;
            float L_pow = L_re * L_re + L_im * L_im; float R_pow = R_re * R_re + R_im * R_im;
            ...
            if (denom_sq > T2_Def::Global::System::MATH_EPSILON_12_CONST * T2_Def::Global::System::MATH_EPSILON_12_CONST) {
                float num_sq = C_re * C_re + C_im * C_im; float coh_sq = num_sq / denom_sq;
                if (coh_sq > 1.0f) coh_sq = 1.0f;
                v_cohSum += sqrtf(coh_sq); v_ipdSum += atan2f(C_im, C_re); v_cohCount++;
            }
        }
        p_audSlot.audio.coh = (v_cohCount > 0) ? (v_cohSum / v_cohCount) : 0.0f;
        p_audSlot.audio.ipd = (v_cohCount > 0) ? (v_ipdSum / v_cohCount) : 0.0f;
    } else {
        p_audSlot.audio.coh = 0.0f; p_audSlot.audio.ipd = 0.0f;
    }
}
...
void CL_T2_FeatureExtractor::_extractTopPeaks(const float* p_power, uint32_t p_bins, float p_sampleRate, T2_Type::ST_SpectralPeak_t* p_outPeaks, uint8_t p_maxCount, float p_ampMin, float p_freqGap) {
    ...
    for (uint32_t i = 1; i < p_bins - 1; i++) {
        if (p_power[i] > p_power[i-1] && p_power[i] > p_power[i+1] && p_power[i] > p_ampMin) {
            ...
            // 삽입 정렬을 통해 크기 역순으로 정렬 상태 유지하며 삽입
...
void CL_T2_FeatureExtractor::_computeMfcc(const float* p_power, uint32_t p_bins, float* p_outMfcc, uint8_t p_chIdx, bool isAudio) {
    ...
    // [핵심 교정 완료] 오디오 특징량 슬롯 분리에 대응하는 독립 오프셋 역산 공식 주입
    const uint32_t singleChDim = T2_Def::AI::Tensor::MFCC_COEFFS_DEF * T2_Def::AI::Tensor::MFCC_COMPONENTS_DEF;
