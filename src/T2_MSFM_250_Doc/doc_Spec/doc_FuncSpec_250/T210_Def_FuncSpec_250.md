...
6.  **NVS 마모 예방 및 WAL 영구 배제**:
    *   파티션 테이블 내 WAL 설정을 차단하고 컴파일 타임 매크로 검증을 적용하여 NVS 기반 관리 구조를 고착시킵니다.

---

## 2. 하드웨어 의존 및 컴파일 타임 매크로 (Compiler Directives)

*   **`SMEA_FLASH_RODATA`**: 필터 계수 및 윈도우 계수 테이블 등 대용량 상수를 Flash ROM 영역(`.rodata` 섹션)에 할당하여 내부 SRAM 소모를 차단합니다. (단, 동적 Notch 필터 등 런타임에 쓰기가 필요한 계수는 RAM 영역에 상주하도록 보장함)
...
*   **Hardware (GPIO 바인딩)**:
    ...
    *   물리 안전 릴레이(인터록) 핀: `PIN_SAFETY_RELAY_CONST` = GPIO 25
*   **Task (FreeRTOS 바인딩)**:
    ...
    *   `LAZY_WRITE_MS_DEF` = 10000ms (NVS Lazy Write 마모 방지 10초 병합 주기)
*   **Path**:
...
*   `ANY_MOTION_LSB_2G_MG` = BMI270 Any-Motion 감지용 LSB 변환 매개변수
...
*   자이로 DSP 런타임 구조체(`ST_GyroDspRuntime_t`)에서 대형 FIR 상태 버퍼 및 계수 배열을 완전히 제거하고 초경량 IIR 4차 바이쿼드 필터 상태로 대체하였습니다.
...
*   `OVERLAP_RATIO` = 0.5f (50% Overlap)
*   `I2S_DMA_CHUNK_SIZE` = `static_cast<uint32_t>(FFT_SIZE_DEF * (1.0f - OVERLAP_RATIO))` (FFT Hop Size = 512 samples)
*   `I2S_DMA_DESC_COUNT` = 6 (WiFi 공존 대응용 백로그 DMA 디스크립터 최소 깊이)
...
이를 통해 가속도 및 자이로 도메인에서는 MFCC 특징 추출 연산을 컴파일 타임에 가지치기(생략)하고, 오디오 도메인에서만 MFCC 연산을 전담하도록 보장합니다.
...
*   **v2.50 (2026-06-21)**:
    *   `PIN_SAFETY_RELAY_CONST` = GPIO 25 릴레이 핀 지정 명시.
    *   WAL 설정 파라미터 완전 삭제 및 NVS Lazy Write 10초(`LAZY_WRITE_MS_DEF = 10000`) 병합 상수 도입.
    *   자이로 4차 HPF/LPF IIR 바이쿼드 계수 구조 반영에 맞춰 FIR 차수 정보 제거 및 경량 상태 반영.
    *   오디오 DMA Chunk Size 수식화 및 `I2S_DMA_DESC_COUNT = 6` 추가.
    *   `T2_General` 네임스페이스 컴파일 타임 Policy Traits 확장 명세 추가.
