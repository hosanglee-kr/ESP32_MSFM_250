...
#include "T249_Calibrator_241.hpp"
#include "esp_log.h"
#include "dsps_wind.h"
...
bool CL_T2_Calibrator::_findLatestCalibFile(char* p_outPath, size_t p_maxLen, bool p_isManual) {
    File v_dir = SD_MMC.open(T2_Config::Path::SD_DIR_RAW_CONST);
    ...
    while (v_file) {
        if (!v_file.isDirectory() && strstr(v_file.name(), v_prefix) != nullptr) {
            ...
            if (v_ctime > v_latestTime) {
                ...
                snprintf(p_outPath, p_maxLen, "%s/%s", T2_Config::Path::SD_DIR_RAW_CONST, v_file.name());
                v_found = true;
            }
        }
...
void CL_T2_Calibrator::_processAuto(const char* p_path) {
    ...
    double v_sumSqVib[T2_Config::System::VIB_AXIS_CONST] = {0.0};
    ...
    while (v_file.available() >= sizeof(T2_Type::UnifiedRawChunk)) {
        ...
        for (uint8_t axis = 0; axis < T2_Config::System::VIB_AXIS_CONST; axis++) {
            for (uint16_t i = 0; i < T2_Config::System::FFT_SIZE_VIB_CONST; i++) {
                v_sumSqVib[axis] += (double)(v_chunk->vib[axis][i] * v_chunk->vib[axis][i]);
            }
        }

        for (uint16_t i = 0; i < T2_Config::System::FFT_SIZE_AUDIO_CONST; i++) {
    ...
    for (uint8_t axis = 0; axis < T2_Config::System::VIB_AXIS_CONST; axis++) {
        float v_rms = (float)sqrt(v_sumSqVib[axis] / (v_totalChunks * T2_Config::System::FFT_SIZE_VIB_CONST));
        if (v_rms > v_baselineVib) v_baselineVib = v_rms;
    }
    float v_baselineAudio = (float)sqrt(v_sumSqAudio / (v_totalChunks * T2_Config::System::FFT_SIZE_AUDIO_CONST));
    ...
    v_cfg.trigger.vib_rms_thresh = fmaxf(v_baselineVib * 3.0f, T2_Config::Decision::RULE_VIB_RMS_THRESH_DEF);
    v_cfg.trigger.audio_rms_thresh = fmaxf(v_baselineAudio * 3.0f, T2_Config::Decision::RULE_AUDIO_RMS_THRESH_DEF);
...
void CL_T2_Calibrator::_processManual(const char* p_path) {
    ...
    float* v_fftWorkBuf = (float*)heap_caps_aligned_alloc(16, T2_Config::System::FFT_SIZE_AUDIO_CONST * 2 * sizeof(float), MALLOC_CAP_SPIRAM);
    float* v_powerAcc = (float*)heap_caps_aligned_alloc(16, (T2_Config::System::FFT_SIZE_AUDIO_CONST / 2 + 1) * sizeof(float), MALLOC_CAP_SPIRAM);
    ...
    memset(v_powerAcc, 0, (T2_Config::System::FFT_SIZE_AUDIO_CONST / 2 + 1) * sizeof(float));
    uint32_t v_chunkCount = 0;
    const uint32_t v_samples = T2_Config::System::FFT_SIZE_AUDIO_CONST;
    ...
    float v_refAmp = sqrtf(fmaxf(v_powerAcc[v_refBin], T2_Config::System::MATH_EPSILON_12_CONST));
    ...
    for (uint16_t i = 0; i <= v_samples / 2; i++) {
        float v_amp = sqrtf(fmaxf(v_powerAcc[i], T2_Config::System::MATH_EPSILON_12_CONST));
    ...
    const uint16_t v_taps = T2_Config::FeatureLimit::FIR_TAPS_CONST;
