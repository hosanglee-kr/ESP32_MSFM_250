...
void CL_T2_FsmManager::begin() {
    ...
    _seqBuilder.init(T2_Config::System::SEQUENCE_FRAMES_MAX_CONST, T2_Config::System::MFCC_DIM_CONST);
    _comm.init();
    ...
    _prcBeamformed  = (float*)heap_caps_aligned_alloc(16, T2_Config::System::FFT_SIZE_AUDIO_CONST * sizeof(float), MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);

	// L/R 다운믹스용 스크래치 버퍼 할당
    _tmpAudL = (float*)heap_caps_aligned_alloc(16, T2_Config::System::FFT_SIZE_AUDIO_CONST * sizeof(float), MALLOC_CAP_SPIRAM);
    _tmpAudR = (float*)heap_caps_aligned_alloc(16, T2_Config::System::FFT_SIZE_AUDIO_CONST * sizeof(float), MALLOC_CAP_SPIRAM);
...
void CL_T2_FsmManager::_acquireData(uint8_t p_idx) {
    ...
    // [교정] 지역 배열(Stack 8KB) 파괴 -> 미리 할당된 PSRAM 버퍼 사용
    uint32_t v_read = _sensor.readAudioChunk(_tmpAudL, _tmpAudR, T2_Config::System::FFT_SIZE_AUDIO_CONST);

    if (v_read == T2_Config::System::FFT_SIZE_AUDIO_CONST) {
        _sensor.readVibFifoBatch(v_raw.vib[0], v_raw.vib[1], v_raw.vib[2], T2_Config::System::FFT_SIZE_VIB_CONST);
        for(int i = 0; i < T2_Config::System::FFT_SIZE_AUDIO_CONST; i++) {
...
void CL_T2_FsmManager::_handleNormalSlot(uint8_t p_slotIdx, T2_Type::UnifiedFeatureSlot& p_slot, T2_Type::UnifiedRawChunk& p_raw) {
	...
	_dsp.processVibration(p_raw.vib[0], p_raw.vib[1], p_raw.vib[2], p_raw.vib[0], p_raw.vib[1], p_raw.vib[2], T2_Config::System::FFT_SIZE_VIB_CONST);

    _extractor.extractAudio(_prcBeamformed, T2_Config::System::FFT_SIZE_AUDIO_CONST, p_slot);
    _extractor.extractVibration(p_raw.vib[0], p_raw.vib[1], p_raw.vib[2], T2_Config::System::FFT_SIZE_VIB_CONST, p_slot);
...
T2_Type::DetectionResult CL_T2_FsmManager::_runHybridDecision(const T2_Type::UnifiedFeatureSlot& p_slot) {
    ...
    for (uint8_t i = 0; i < T2_Config::System::VIB_AXIS_CONST; i++) {
