/* ============================================================================
 * File: T270_Commu_246.cpp
 * Summary: 멀티플렉싱 통신 엔진 구현부 (v245)
 * ========================================================================== */
...
bool CL_T2_Communicator::init() {
    ...
    ESP_LOGI(TAG, "Communication Engine v245 Ready.");
    return true;
}
...
void CL_T2_Communicator::_initWebHandlers() {
    ...
    _server.on("/api/files", HTTP_GET, [](AsyncWebServerRequest* p_req) {
        if (LittleFS.exists("/sys/runtime_idx_246.json")) {
            p_req->send(LittleFS, "/sys/runtime_idx_246.json", "application/json");
        } else {
...
bool CL_T2_Communicator::publishResultMqtt(const T2_Type::ST_UnifiedFeatureSlot_t& p_slot, T2_Type::EM_DetectionResult_t p_result) {
    ...
    char v_payload[384]; // 버퍼 크기 증가 (자이로 데이터가 추가되었으므로)
