...
public:
    ...
    // 함수설명: 센서 설정을 인자로 받아 가속도/자이로 및 마이크 수집 모듈들을 초기화합니다. (p_sysCfg: 글로벌 설정, p_accCfg: 가속도 설정, p_gyrCfg: 자이로 설정, p_audCfg: 마이크 설정, 반환값: 성공 여부)
    ...
    // 함수설명: 데이터 수집 및 DMA 전송 과정을 일시 정지시킵니다.
    void pause();

    // 함수설명: 일시 정지 상태에서 데이터 수집 및 DMA 전송 과정을 재개합니다.
    void resume();

    // 함수설명: 마이크용 DMA 버퍼 및 내부 I2S 대기 큐를 완전히 비워 초기화합니다.
    void clearAudioBuffer();

    // 함수설명: 센서 하드웨어를 재부팅하여 통신 상태를 리셋합니다.
    void resetHardware();

    // 함수설명: 마이크 센서로부터 16비트 오디오 샘플 청크를 가져와 채널 분리를 수행합니다. (p_outL: L채널 출력 버퍼, p_outR: R채널 출력 버퍼, p_reqSamples: 요구 샘플 개수, 반환값: 수신 완료된 샘플 개수)
    uint32_t readAudioChunk(float* p_outL, float* p_outR, uint32_t p_reqSamples);

    // 함수설명: BMI270 센서의 FIFO로부터 진동 원시 데이터를 다량으로 수집하여 가변 정밀도로 변환 적용합니다. (p_accX/Y/Z: 가속도 축별 출력 버퍼, p_gyrX/Y/Z: 자이로 축별 출력 버퍼, p_maxFrames: 최대 요청 프레임 수, 반환값: 변환 완료된 프레임 개수)
    ...
    // 함수설명: 센서 FIFO를 읽고 내부 축별 누적 링버퍼에 적재합니다. (반환값: 축적 완료된 진동 데이터 프레임 수)
    uint16_t accumulateFifo();

    // 함수설명: 누적된 가속도 데이터 링버퍼에서 지정 수량만큼의 샘플을 꺼내옵니다. (p_outX/Y/Z: 출력 버퍼, p_reqCount: 요청 개수, 반환값: 복사 완료된 개수)
    uint16_t getAccumulatedAccel(float* p_outX, float* p_outY, float* p_outZ, uint16_t p_reqCount);

    // 함수설명: 누적된 자이로 데이터 링버퍼에서 지정 수량만큼의 샘플을 꺼내옵니다. (p_outX/Y/Z: 출력 버퍼, p_reqCount: 요청 개수, 반환값: 복사 완료된 개수)
    uint16_t getAccumulatedGyro(float* p_outX, float* p_outY, float* p_outZ, uint16_t p_reqCount);

    // 함수설명: 누적 대기 상태인 가속도 데이터 프레임 개수를 반환합니다. (반환값: 누적 프레임 수)
    uint16_t getAccumulatedAccelCount() const { return _accumCount; }

    // 함수설명: 누적 대기 상태인 자이로 데이터 프레임 개수를 반환합니다. (반환값: 누적 프레임 수)
    uint16_t getAccumulatedGyroCount() const { return _accumGCount; }

    // 함수설명: BMI270 칩 내부 온도 센서에서 섭씨 온도를 읽어와 반환합니다. (반환값: 섭씨 온도값)
    float readTemperatureSensor();

    // 함수설명: 가속도 센서의 3축 소프트웨어 오프셋 캘리브레이션을 진행하고 결과를 파일로 저장합니다. (반환값: 성공 여부)
    bool runAccelCalibration();

    // 함수설명: 자이로 센서의 3축 소프트웨어 오프셋 캘리브레이션을 진행하고 결과를 파일로 저장합니다. (반환값: 성공 여부)
    bool runGyroCalibration();

    // 함수설명: 이전에 파일시스템에 저장되어 있던 가속도 캘리브레이션 설정값을 로드하여 적용합니다. (반환값: 복원 성공 여부)
    bool applyStoredAccelCalibration();

    // 함수설명: 이전에 파일시스템에 저장되어 있던 자이로 캘리브레이션 설정값을 로드하여 적용합니다. (반환값: 복원 성공 여부)
    bool applyStoredGyroCalibration();

    // 함수설명: 움직임 감지 시 WDT 해제 또는 부팅 트리거가 유발되도록 Any-Motion 인터럽트 설정을 활성화합니다. (p_threshG: 감지 가속도 임계치 G, p_duration: 최소 충족 틱 수, 반환값: 적용 성공 여부)
    bool enableWakeOnMotion(float p_threshG, uint16_t p_duration);

    // 함수설명: 현재 센서 수집 모듈들의 동작 상태 문자열을 반환합니다. (반환값: 상태 문자열 포인터)
