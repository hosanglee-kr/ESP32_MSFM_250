...
// 함수설명: 센서 엔진 클래스 생성자이며, 멤버 변수들을 기본값으로 초기화합니다. (p_spiBus: SPIClass 참조)
...
// 함수설명: 설정 정보를 바탕으로 BMI270 관성 센서 및 I2S 마이크 구동 모듈들을 초기화합니다. (p_sysCfg: 글로벌 설정, p_accCfg: 가속도 설정, p_gyrCfg: 자이로 설정, p_audCfg: 마이크 설정, 반환값: 초기화 성공 여부)
...
// 함수설명: 센서 관성 칩을 전력 절감 상태로 전환하고 마이크의 I2S 수집도 중단시킵니다.
...
// 함수설명: 대기 전력 모드에 들어간 관성 칩 및 마이크 I2S 포트를 깨워 즉시 수집을 재개합니다.
...
// 함수설명: 센서 칩 리셋 핀 트리거 및 제어 플래그 변수들을 초기화 상태로 복구시킵니다.
...
// 함수설명: 마이크용 DMA 버퍼 영역을 무음 데이터로 덮어씌워 강제 클리어합니다.
...
// 함수설명: I2S DMA 버퍼로부터 스테레오 혹은 단일 채널 사운드 데이터를 가져와 정규화 분배합니다. (p_outL: L채널 대상 버퍼, p_outR: R채널 대상 버퍼, p_reqSamples: 요구 개수, 반환값: 실제 획득 샘플 수)
...
// 함수설명: BMI270 FIFO 버스트 바이트를 읽어 가속도 및 자이로 수집 버퍼에 분리 수집합니다. (p_accX/Y/Z: 가속도 대상 버퍼군, p_gyrX/Y/Z: 자이로 대상 버퍼군, p_maxFrames: 최대 수집 프레임 제한, 반환값: 읽은 프레임 수)
...
// 함수설명: 주기적으로 FIFO를 읽어 가속도 및 자이로 링버퍼에 비동기 누적 적재합니다. (반환값: 누적시킨 프레임 수)
        ...
        if (v_header == Imu::Hardware::FIFO_HEADER_ACCEL_CONST) {
            ...
            if (v_accCount < v_framesToRead) {
                if (_accelAxisMask & (1 << 0)) {
                    int16_t v_rawX = (int16_t)((v_fifoData[v_idx + 1] << 8) | v_fifoData[v_idx]);
                    _accumX[_accumWriteIdx] = ((float)v_rawX * _lsbToG - _accOffsetX) * _accGainX;
                } else {
                    _accumX[_accumWriteIdx] = 0.0f;
                }
                
                if (_accelAxisMask & (1 << 1)) {
                    int16_t v_rawY = (int16_t)((v_fifoData[v_idx + 3] << 8) | v_fifoData[v_idx + 2]);
                    _accumY[_accumWriteIdx] = ((float)v_rawY * _lsbToG - _accOffsetY) * _accGainY;
                } else {
                    _accumY[_accumWriteIdx] = 0.0f;
                }

                if (_accelAxisMask & (1 << 2)) {
                    int16_t v_rawZ = (int16_t)((v_fifoData[v_idx + 5] << 8) | v_fifoData[v_idx + 4]);
                    _accumZ[_accumWriteIdx] = ((float)v_rawZ * _lsbToG - _accOffsetZ) * _accGainZ;
                } else {
                    _accumZ[_accumWriteIdx] = 0.0f;
                }
            ...
            if (_gyroEnabled && v_gyrCount < v_framesToRead) {
                if (_gyroAxisMask & (1 << 0)) {
                    int16_t v_rawGX = (int16_t)((v_fifoData[v_idx + 1] << 8) | v_fifoData[v_idx]);
                    _accumGX[_accumGWriteIdx] = ((float)v_rawGX * _lsbToDps - _gyrOffsetX) * _gyrGainX;
                } else {
                    _accumGX[_accumGWriteIdx] = 0.0f;
                }

                if (_gyroAxisMask & (1 << 1)) {
                    int16_t v_rawGY = (int16_t)((v_fifoData[v_idx + 3] << 8) | v_fifoData[v_idx + 2]);
                    _accumGY[_accumGWriteIdx] = ((float)v_rawGY * _lsbToDps - _gyrOffsetY) * _gyrGainY;
                } else {
                    _accumGY[_accumGWriteIdx] = 0.0f;
                }

                if (_gyroAxisMask & (1 << 2)) {
                    int16_t v_rawGZ = (int16_t)((v_fifoData[v_idx + 5] << 8) | v_fifoData[v_idx + 4]);
                    _accumGZ[_accumGWriteIdx] = ((float)v_rawGZ * _lsbToDps - _gyrOffsetZ) * _gyrGainZ;
                } else {
                    _accumGZ[_accumGWriteIdx] = 0.0f;
                }
...
// 함수설명: 내부 가속도 링버퍼로부터 지정한 샘플 개수만큼 순차 반환 인출합니다. (p_outX/Y/Z: 대상 버퍼군, p_reqCount: 요청 개수, 반환값: 실제 획득 개수)
...
// 함수설명: 내부 자이로 링버퍼로부터 지정한 샘플 개수만큼 순차 반환 인출합니다. (p_outX/Y/Z: 대상 버퍼군, p_reqCount: 요청 개수, 반환값: 실제 획득 개수)
...
// 함수설명: 가속도 센서의 HW 리트림 자가 교정을 수행하고 산출된 소프트웨어 오프셋 편차를 플래시 파일에 저장합니다. (반환값: 성공 여부)
...
// 함수설명: 자이로 센서의 HW 리트림 자가 교정을 수행하고 산출된 소프트웨어 오프셋 편차를 플래시 파일에 저장합니다. (반환값: 성공 여부)
...
// 함수설명: 플래시에서 로드한 가속도 캘리브레이션 JSON 데이터를 읽어와 센서 엔진 오프셋 변수에 적용합니다. (반환값: 복원 성공 여부)
...
// 함수설명: 플래시에서 로드한 자이로 캘리브레이션 JSON 데이터를 읽어와 센서 엔진 오프셋 변수에 적용합니다. (반환값: 복원 성공 여부)
...
// 함수설명: 지정한 움직임 감지 한계를 초과하면 INT1 핀으로 인터럽트 신호가 나가도록 BMI270에 모션 설정을 등록합니다. (p_threshG: 가속도 기준 G, p_duration: 최소 충족 틱 수, 반환값: 활성화 성공 여부)
...
// 함수설명: 입력된 범위 G를 BMI270 라이브러리 가속도 대역 열거형 값으로 변환합니다. (p_rangeG: 요청 범위 G, 반환값: 매핑된 레지스터 값)
...
// 함수설명: 입력된 자이로 범위 dps를 BMI270 라이브러리 자이로 대역 열거형 값으로 변환합니다. (p_rangeDps: 요청 범위 dps, 반환값: 매핑된 레지스터 값)
...
// 함수설명: SPI 버스를 점유하여 센서 레지스터로부터 데이터를 버스트 수신합니다. (p_reg: 대상 레지스터 번호, p_data: 수신용 배열 버퍼, p_len: 크기 byte, 반환값: 성공 여부)
...
// 함수설명: SPI 버스를 점유하여 센서 레지스터에 데이터를 전송 기록합니다. (p_reg: 대상 레지스터 번호, p_data: 전송할 바이트 배열, p_len: 크기 byte, 반환값: 성공 여부)
...
// 함수설명: BMI270 레지스터에서 온도 원시 값을 읽어 환산 상수를 적용한 섭씨 온도를 반환합니다. (반환값: 변환 완료된 섭씨 온도)
