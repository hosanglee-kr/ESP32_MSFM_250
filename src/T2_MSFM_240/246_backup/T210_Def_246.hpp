...
/* ============================================================================
 * File: T210_Def_246.hpp
 * Summary: T240 4-Tier 시스템 상수 정의 및 동적 파라미터 SSOT
 * ============================================================================
 * [구현 원칙 및 아키텍처 가이드라인]
...
namespace Global {
    namespace System {
        inline constexpr char const* VERSION_STR           = "T240_v245_1";       // 시스템 펌웨어 식별 버전
    ...
    namespace Path {
        ...
        inline constexpr char const* FILE_CFG_JSON_CONST   = "/sys/runtime_cfg_246.json";     // 런타임 동적 설정값
        inline constexpr char const* FILE_CFG_TMP_CONST    = "/sys/config_246.tmp";           // 설정 임시 파일 경로
        inline constexpr char const* FILE_IDX_JSON_CONST   = "/sys/runtime_idx_246.json";     // 인덱스 파일 경로
        inline constexpr char const* FILE_IDX_TMP_CONST    = "/sys/index_246.tmp";            // 인덱스 임시 파일 경로
    ...
    namespace Net {
        ...
        inline constexpr char const*  MQTT_ID_DEF             = "T240_Edge";         // MQTT 클라이언트 ID
        inline constexpr char const*  MQTT_TOPIC_DEF             = "smea/t240";         // MQTT 최상위 토픽
        inline constexpr char const*  MQTT_LWT_DEF             = "smea/t240/lwt";     // 유언(LWT) 토픽
    ...
    namespace Storage {
        ...
        inline constexpr char const*  SD_DIR_RAW_CONST     = "/t240_data/raw";  // 원시 파형 저장 경로
        inline constexpr char const*  SD_DIR_BIN_CONST     = "/t240_data/bin";  // 특징량 바이너리 저장 경로
    }  // namespace Storage
    ...
    namespace Dsp {
        ...
        // inline constexpr int   MEDIAN_WINDOW_DEF         = 3;          // [삭제] 중복 방지: 개별 ST_Dsp_Config에서 직접 정의하여 제어
...
namespace Imu {
    namespace Hardware {
        ...
        inline constexpr uint8_t  PIN_INT1_CONST           = 11;        // [수정] FIFO Watermark 인터럽트 전용 수신 핀 (Core 0 바인딩)
        inline constexpr uint8_t  PIN_INT2_CONST           = 12;        // [신규] Any-Motion 전용 수신 핀 (Core 1 / 외부 Ext0 딥슬립 웨이크업 바인딩)
...
namespace Accel {
    ...
    namespace Calib {
        inline constexpr char const*  FILE_JSON_CONST     = "/sys/acc_calib_246.json";  // 가속도 영점 보정 파일
...
namespace Gyro {
    ...
    namespace Calib {
        inline constexpr char const* FILE_JSON_CONST         = "/sys/gyr_calib_246.json";
        inline constexpr uint16_t    TARGET_SAMPLES_DEF      = 200; // 자이로 노이즈 평균화를 위해 더 많은 샘플 권장
        inline constexpr uint16_t    RETRY_MAX_CONST         = 15;
        inline constexpr uint32_t    LOOP_DELAY_MS_CONST     = 20;
    }
}  // namespace Gyro
...
namespace Audio {
    namespace Hardware {
        ...
        inline constexpr int      DMA_BUF_LEN_OPT_CONST    = 512;               // 쳕크 당 샘플 수 (512/42000 ≈ 12.2ms 주기)
    ...
    namespace Calib {
        inline constexpr char const*  FILE_JSON_CONST        = "/sys/aud_calib_246.json";  // 마이크 EQ 보정 파일
...
namespace Imu {
    namespace Hardware {
        // --- [수정] 인터럽트 핀 물리 분리 및 다중 바인딩 ---
        inline constexpr uint8_t  PIN_CS_CONST             = 10;        // BMI270 SPI CS 핀
        inline constexpr uint8_t  PIN_INT1_CONST           = 11;        // [수정] FIFO Watermark 인터럽트 전용 수신 핀 (Core 0 바인딩)
        inline constexpr uint8_t  PIN_INT2_CONST           = 12;        // [신규] Any-Motion 전용 수신 핀 (Core 1 / 외부 Ext0 딥슬립 웨이크업 바인딩)
        inline constexpr uint32_t SPI_FREQ_HZ_CONST        = 10000000;  // SPI 버스 속도 (10MHz)

        // --- [추가] BMI270 내부 인터럽트 및 상태 레지스터 주소 (매직넘버 제로화) ---
        inline constexpr uint8_t  REG_INT_STATUS_0_ADDR    = 0x1C;      // [신규] 인터럽트 상태 레지스터 0 (Motion 감지 확인용)
        inline constexpr uint8_t  REG_INT_STATUS_1_ADDR    = 0x1D;      // [신규] 인터럽트 상태 레지스터 1 (FIFO Watermark 확인용)
        inline constexpr uint8_t  REG_FIFO_WTM_STATUS_BIT  = 0x02;      // [신규] Status 1 내 FIFO Watermark 비트 마스크 (0x02)
        inline constexpr uint8_t  REG_ANY_MOTION_STATUS_BIT = 0x20;     // [신규] Status 0 내 Any-Motion 감지 비트 마스크 (0x20)

        // --- [유지 및 검증] FIFO 제어 파라미터 상숫값 ---
        inline constexpr uint16_t FIFO_WATERMARK_LIMIT     = 40;        // 40세트 누적 시 워터마크 발생 (약 25ms 간격)
        inline constexpr uint16_t FIFO_BATCH_SIZE_MAX      = 128;       // FIFO 버스트 리드 버퍼 크기 정적 상한선
        inline constexpr uint16_t FIFO_BATCH_SIZE_DEF      = 40;        // 기본 watermark 매핑 데이터 인출 크기
        ...
        // --- [추가] Any-Motion 제어 레지스터 상세 파라미터 ---
        inline constexpr float    ANY_MOTION_LSB_2G_MG     = 0.48f;     // Any-motion 임계치 LSB (2G 대역 기준)
        inline constexpr uint32_t STARTUP_DELAY_MS_CONST   = 5;         // 활성화 안정화 대기 시간
        inline constexpr uint8_t  REG_TEMP_MSB_CONST       = 0x22;      // BMI270 칩 온도 레지스터 및 변환 상수
        inline constexpr int16_t  TEMP_INVALID_CONST       = -32768;    // 온도 레지스터 무효값 판정
        inline constexpr float    TEMP_SCALE_CONST         = 512.0f;    // 온도 LSB → ℃ 변환 계수
