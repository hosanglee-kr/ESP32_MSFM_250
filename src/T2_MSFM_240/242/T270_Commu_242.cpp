 ...
 * - 갱신: 모든 지연 구간에 vTaskDelay() 적용 및 핫스왑 API(Preview/Save) 복원.
...
CL_T2_Communicator::CL_T2_Communicator()
    : _server(80), _ws(T2_Def::Net::WS_URI_CONST) {}
...
bool CL_T2_Communicator::init() {
        ...
        for (uint8_t i = 0; i < T2_Def::Net::WIFI_MULTI_MAX_CONST; i++) {
            if (strlen(v_cfg.wifi.multi_ssid[i]) > 0) {
                ...
                if (WiFi.status() == WL_CONNECTED) {
                    ...
                    configTzTime(T2_Def::Net::TZ_INFO_CONST, T2_Def::Net::NTP_SERVER_1_CONST, "time.google.com");
	...
	if (v_cfg.mqtt.enable && strlen(v_cfg.mqtt.broker) > 0) {
        ...
        snprintf(v_lwtTopic, sizeof(v_lwtTopic), "%s/status/lwt", v_cfg.mqtt.topic_root); // 동적 결합
		...
		v_mqttCfg.keepalive = 60; // MQTT 3.1.1 네이티브 Keep-Alive 강제 (60초)
	...
	// WebSocket 접속 상태 모니터링 이벤트 핸들러 추가
...
void CL_T2_Communicator::_initWebHandlers() {
	// 전역 CORS 헤더 추가 (모든 OPTIONS Preflight 및 응답에 자동 적용)
    ...
    _server.on("/api/files", HTTP_GET, [](AsyncWebServerRequest* p_req) {
        if (LittleFS.exists("/sys/runtime_idx_242.json")) {
            // 스토리지 엔진이 갱신하는 인덱스 파일을 그대로 서빙 (Zero-copy 효율)
            p_req->send(LittleFS, "/sys/runtime_idx_242.json", "application/json");
        } else {
            p_req->send(404, "application/json", "{\"ok\":false, \"msg\":\"No files found.\"}");
        }
    });

	    // Web OTA (Over-The-Air) 펌웨어 업데이트 라우터
...
bool CL_T2_Communicator::publishResultMqtt(const T2_Type::UnifiedFeatureSlot& p_slot, T2_Type::DetectionResult p_result) {
	...
	char v_pubTopic[128];
    snprintf(v_pubTopic, sizeof(v_pubTopic), "%s/res", v_cfg.mqtt.topic_root); // 동적 토픽
...
void CL_T2_Communicator::_mqttEventHandler(void* p_handlerArgs, esp_event_base_t p_base, int32_t p_eventId, void* p_eventData) {
    ...
    switch ((esp_mqtt_event_id_t)p_eventId) {
        ...
        case MQTT_EVENT_DATA: {
            // 수신된 토픽이 CMD 토픽과 정확히 일치하는지 먼저 검사 (보안 방어벽)
