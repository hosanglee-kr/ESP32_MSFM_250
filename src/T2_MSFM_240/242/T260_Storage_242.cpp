...
bool CL_T2_StorageManager::init() {
    // SD카드 마운트
    if (!SD_MMC.begin(T2_Def::Path::MOUNT_SD_CONST, true)) {
        ESP_LOGE(TAG, "SD Card Mount Failed!");
        _ioError = true;
        return false;
    }

    if (!SD_MMC.exists(T2_Def::Path::SD_DIR_BIN_CONST)) SD_MMC.mkdir(T2_Def::Path::SD_DIR_BIN_CONST);
    if (!SD_MMC.exists(T2_Def::Path::SD_DIR_RAW_CONST)) SD_MMC.mkdir(T2_Def::Path::SD_DIR_RAW_CONST);
    ...
    xTaskCreatePinnedToCore(_storageTaskProc, "StrgTask", T2_Def::Task::STORAGE_STACK_CONST,
                            this, T2_Def::Task::STORAGE_PRIO_CONST, &_hStorageTask, T2_Def::Task::CORE_PROCESS_CONST);
...
void CL_T2_StorageManager::_allocatePreBuffer() {
    ...
    _preCapacity = v_cfg.storage.pre_trigger_sec * (T2_Def::System::RATE_AUDIO_CONST / T2_Def::System::FFT_SIZE_AUDIO_CONST);
...
bool CL_T2_StorageManager::openSession(const char* p_prefix, const char* p_overrideDir) {
    ...
    if (p_overrideDir != nullptr) {
        ...
        // [복원/교정] BIN 폴더 계층화
        char v_dirBin[T2_Def::StorageLimit::MAX_PATH_LEN_CONST];
        snprintf(v_dirBin, sizeof(v_dirBin), "%s/%04d/%02d/%02d", T2_Def::Path::SD_DIR_BIN_CONST, v_tinfo.tm_year + 1900, v_tinfo.tm_mon + 1, v_tinfo.tm_mday);
        if (!SD_MMC.exists(v_dirBin)) { /* 상위 폴더 생성 로직 생략 (기존 코드 활용) */ SD_MMC.mkdir(v_dirBin); }

        // [복원/교정] RAW 폴더 계층화 (FAT32 O(N) 붕괴 완벽 차단)
        char v_dirRaw[T2_Def::StorageLimit::MAX_PATH_LEN_CONST];
        snprintf(v_dirRaw, sizeof(v_dirRaw), "%s/%04d/%02d/%02d", T2_Def::Path::SD_DIR_RAW_CONST, v_tinfo.tm_year + 1900, v_tinfo.tm_mon + 1, v_tinfo.tm_mday);
    ...
    // 선할당 후 초기 헤더 주입
    _preAllocateFile(_activeFile, T2_Def::StorageLimit::PREALLOC_BYTES_CONST);
    _preAllocateFile(_rawFile, T2_Def::StorageLimit::PREALLOC_BYTES_CONST);
...
void CL_T2_StorageManager::_appendIndexItem() {
    ...
    if (_indexCount >= T2_Def::StorageLimit::MAX_ROTATE_LIST_CONST) {
        ...
        for (uint16_t i = 0; i < T2_Def::StorageLimit::MAX_ROTATE_LIST_CONST - 1; i++) {
            _indexItems[i] = _indexItems[i + 1];
        }
        _indexCount--;
    }

    strlcpy(_indexItems[_indexCount].path, _activePath, T2_Def::StorageLimit::MAX_PATH_LEN_CONST);
    strlcpy(_indexItems[_indexCount].raw_path, _activeRawPath, T2_Def::StorageLimit::MAX_PATH_LEN_CONST);
...
bool CL_T2_StorageManager::_writeIndexFileAtomic() {
    File v_tmp = LittleFS.open("/sys/index_242.tmp", "w");
    ...
    LittleFS.remove("/sys/runtime_idx_242.json");
    LittleFS.rename("/sys/index_242.tmp", "/sys/runtime_idx_242.json");
    return true;
}

bool CL_T2_StorageManager::_loadIndexJson() {
    if (!LittleFS.exists("/sys/runtime_idx_242.json")) return false;

    File v_file = LittleFS.open("/sys/runtime_idx_242.json", "r");
    ...
    for (JsonObjectConst v_obj : v_arr) {
        if (_indexCount >= T2_Def::StorageLimit::MAX_ROTATE_LIST_CONST) break;
        strlcpy(_indexItems[_indexCount].path, v_obj["path"] | "", T2_Def::StorageLimit::MAX_PATH_LEN_CONST);
        strlcpy(_indexItems[_indexCount].raw_path, v_obj["raw_path"] | "", T2_Def::StorageLimit::MAX_PATH_LEN_CONST);
...
//     char v_dirPath[T2_Def::StorageLimit::MAX_PATH_LEN_CONST];
//     snprintf(v_dirPath, sizeof(v_dirPath), "%s/%04d/%02d/%02d",
//              T2_Def::Path::SD_DIR_BIN_CONST, v_tinfo.tm_year + 1900, v_tinfo.tm_mon + 1, v_tinfo.tm_mday);

//     // O(N) 디렉토리 탐색 병목 회피를 위해 하위 뎁스(Depth)만 검사 후 순차 생성
//     if (!SD_MMC.exists(v_dirPath)) {
//         char v_tmp[128];
//         snprintf(v_tmp, sizeof(v_tmp), "%s/%04d", T2_Def::Path::SD_DIR_BIN_CONST, v_tinfo.tm_year + 1900);
//         if(!SD_MMC.exists(v_tmp)) SD_MMC.mkdir(v_tmp);

//         snprintf(v_tmp, sizeof(v_tmp), "%s/%04d/%02d", T2_Def::Path::SD_DIR_BIN_CONST, v_tinfo.tm_year + 1900, v_tinfo.tm_mon + 1);
//         if(!SD_MMC.exists(v_tmp)) SD_MMC.mkdir(v_tmp);
...
//              T2_Def::Path::SD_DIR_RAW_CONST, _currentPrefix, (uint64_t)time(NULL), _rotationSubSeq);
