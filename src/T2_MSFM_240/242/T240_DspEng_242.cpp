...
bool CL_T2_DspEngine::init() {
    ...
    dsps_wind_hann_f32(_audWindow, T2_Def::System::FFT_SIZE_AUDIO_CONST);
    dsps_wind_hann_f32(_vibWindow, T2_Def::System::FFT_SIZE_VIB_CONST);
...
void CL_T2_DspEngine::reloadFilters() {
    ...
    // 1. Notch 필터 갱신
    _calcNotchCoeffs(v_cfg.dsp.notch.target_freq_hz, v_cfg.dsp.notch.q_factor, _notch60Coeffs, T2_Def::System::RATE_AUDIO_CONST);

    // 2. FIR 필터 계수 동적 갱신
    _generateFirLpfWindowedSinc(_firLpfAudioCoeffs, T2_Def::FeatureLimit::FIR_TAPS_CONST, v_cfg.dsp.fir_lpf.cutoff_hz, (float)T2_Def::System::RATE_AUDIO_CONST);
    dsps_fir_init_f32(&_firInstLpfAudio, _firLpfAudioCoeffs, _firStateLpfAudio, T2_Def::FeatureLimit::FIR_TAPS_CONST);

    _generateFirHpfWindowedSinc(_firHpfAudioCoeffs, T2_Def::FeatureLimit::FIR_TAPS_CONST, v_cfg.dsp.fir_hpf.cutoff_hz, (float)T2_Def::System::RATE_AUDIO_CONST);
    dsps_fir_init_f32(&_firInstHpfAudio, _firHpfAudioCoeffs, _firStateHpfAudio, T2_Def::FeatureLimit::FIR_TAPS_CONST);

    // 3. IIR 필터 계수 갱신
    _calcIirHpCoeffs(v_cfg.dsp.iir_hpf.cutoff_hz, v_cfg.dsp.iir_hpf.q_factor, _iirHpfCoeffs, T2_Def::System::RATE_AUDIO_CONST);
    _calcIirLpCoeffs(v_cfg.dsp.iir_lpf.cutoff_hz, v_cfg.dsp.iir_lpf.q_factor, _iirLpfCoeffs, T2_Def::System::RATE_AUDIO_CONST);
}
...
void CL_T2_DspEngine::processAcoustic(const float* p_audL, const float* p_audR, float* p_outBeam) {
    ...
    // 1. LTI 빔포밍 (L+R 병합)
    for (uint32_t i = 0; i < T2_Def::System::FFT_SIZE_AUDIO_CONST; i++) {
        p_outBeam[i] = (p_audL[i] * v_gainL) + (p_audR[i] * v_gainR);
    }

    // 2. [복원] Median Filter
    if (v_cfg.dsp.median_enabled) {
        _applyMedianFilter(p_outBeam, _medianHistAudio, v_cfg.dsp.median_window, T2_Def::System::FFT_SIZE_AUDIO_CONST);
    }
    ...
    if (v_cfg.dsp.notch.enabled) {
        dsps_biquad_f32(v_src, v_dst, T2_Def::System::FFT_SIZE_AUDIO_CONST, _notch60Coeffs, _notch60State);
        v_src = _audioBufA; v_dst = _audioBufB;
    }
    if (v_cfg.dsp.fir_hpf.enabled) {
        dsps_fir_f32(&_firInstHpfAudio, v_src, v_dst, T2_Def::System::FFT_SIZE_AUDIO_CONST);
        float* temp = v_src; v_src = v_dst; v_dst = temp;
    }
    if (v_cfg.dsp.fir_lpf.enabled) {
        dsps_fir_f32(&_firInstLpfAudio, v_src, v_dst, T2_Def::System::FFT_SIZE_AUDIO_CONST);
        float* temp = v_src; v_src = v_dst; v_dst = temp;
    }
    ...
    if (v_cfg.dsp.iir_hpf.enabled) {
        dsps_biquad_f32(v_src, v_dst, T2_Def::System::FFT_SIZE_AUDIO_CONST, _iirHpfCoeffs, _iirHpfState);
        float* temp = v_src; v_src = v_dst; v_dst = temp;
    }
    if (v_cfg.dsp.iir_lpf.enabled) {
        dsps_biquad_f32(v_src, v_dst, T2_Def::System::FFT_SIZE_AUDIO_CONST, _iirLpfCoeffs, _iirLpfState);
        float* temp = v_src; v_src = v_dst; v_dst = temp;
    }


    if (v_src != p_outBeam) memcpy(p_outBeam, v_src, T2_Def::System::FFT_SIZE_AUDIO_CONST * sizeof(float));

    // 4. [복원] Pre-emphasis & Noise Gate & Windowing
    if (v_cfg.dsp.preemphasis_enable) _applyPreEmphasis(p_outBeam, _prevPreEmpAudio, T2_Def::System::FFT_SIZE_AUDIO_CONST, v_cfg.dsp.preemphasis_alpha);
    if (v_cfg.dsp.noise.enable_gate) _applyNoiseGate(p_outBeam, T2_Def::System::FFT_SIZE_AUDIO_CONST, v_cfg.dsp.noise.gate_threshold_abs);

    for (uint32_t i = 0; i < T2_Def::System::FFT_SIZE_AUDIO_CONST; i++) p_outBeam[i] *= _audWindow[i];
}
...
void CL_T2_DspEngine::_generateFirLpfWindowedSinc(float* p_coeffs, uint16_t p_taps, float p_cutoffHz, float p_sampleRate) {
    ...
    alignas(16) float v_win[T2_Def::FeatureLimit::FIR_TAPS_CONST] = {0};
    ...
    for (uint16_t i = 0; i < p_taps; i++) {
        ...
        if (fabsf(v_x) < T2_Def::System::MATH_EPSILON_12_CONST) p_coeffs[i] = 2.0f * v_normalizedCutoff;
