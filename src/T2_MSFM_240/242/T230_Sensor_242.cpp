...
bool CL_T2_SensorEngine::init(const T2_Type::ST_Config_Sensor& p_cfg) {
    ...
    // [1] BMI270 Init
    int8_t v_rslt = _bmi.beginSPI(T2_Def::Hardware::PIN_BMI_CS_CONST, 10000000, _spi);
    if (v_rslt != BMI2_OK) {
    ...
    i2s_config_t v_i2sConfig = {
        ...
        .sample_rate = T2_Def::System::RATE_AUDIO_CONST,
        .bits_per_sample = I2S_BITS_PER_SAMPLE_32BIT,
        .channel_format = I2S_CHANNEL_FMT_RIGHT_LEFT,
        .communication_format = I2S_COMM_FORMAT_STAND_I2S,
        .intr_alloc_flags = ESP_INTR_FLAG_LEVEL3,
        .dma_buf_count = 8,
        .dma_buf_len = T2_Def::System::FFT_SIZE_AUDIO_CONST,
        .use_apll = true,
    ...
    i2s_pin_config_t v_pinConfig = {
        .bck_io_num = T2_Def::Hardware::PIN_I2S_BCLK_CONST,
        .ws_io_num = T2_Def::Hardware::PIN_I2S_WS_CONST,
        .data_out_num = I2S_PIN_NO_CHANGE,
        .data_in_num = T2_Def::Hardware::PIN_I2S_DIN_CONST
    };

    if (i2s_driver_install((i2s_port_t)T2_Def::Hardware::I2S_PORT_NUM_CONST, &v_i2sConfig, 0, NULL) != ESP_OK) {
        ESP_LOGE(TAG, "I2S driver install failed");
        strlcpy(_statusText, "I2S_ERR", sizeof(_statusText));
        return false;
    }

    if (i2s_set_pin((i2s_port_t)T2_Def::Hardware::I2S_PORT_NUM_CONST, &v_pinConfig) != ESP_OK) {
...
void CL_T2_SensorEngine::pause() {
    if (_isPaused) return;
    if (_isI2sInit) i2s_stop((i2s_port_t)T2_Def::Hardware::I2S_PORT_NUM_CONST);
    if (_isBmiInit) {
...
void CL_T2_SensorEngine::resume() {
    ...
    if (_isI2sInit) {
        clearAudioBuffer();
        i2s_start((i2s_port_t)T2_Def::Hardware::I2S_PORT_NUM_CONST);
    }
    _isPaused = false;
    strlcpy(_statusText, "RUNNING", sizeof(_statusText));
}
...
void CL_T2_SensorEngine::clearAudioBuffer() {
    ...
    i2s_zero_dma_buffer((i2s_port_t)T2_Def::Hardware::I2S_PORT_NUM_CONST);
}
...
uint32_t CL_T2_SensorEngine::readAudioChunk(float* p_outL, float* p_outR, uint32_t p_reqSamples) {
    ...
    uint32_t v_samplesToRead = (p_reqSamples > T2_Def::System::FFT_SIZE_AUDIO_CONST) ?
                               T2_Def::System::FFT_SIZE_AUDIO_CONST : p_reqSamples;
    size_t v_bytesRead = 0;
    size_t v_bytesToRead = v_samplesToRead * 2 * sizeof(int32_t);

    esp_err_t v_res = i2s_read((i2s_port_t)T2_Def::Hardware::I2S_PORT_NUM_CONST, _dmaBuffer, v_bytesToRead, &v_bytesRead, portMAX_DELAY);
...
bool CL_T2_SensorEngine::runCalibration() {
    ...
    File v_file = LittleFS.open(T2_Def::Path::FILE_BMI_CALIB_CONST, "w");
    if (v_file) {
...
bool CL_T2_SensorEngine::applyStoredCalibration() {
    if (!LittleFS.exists(T2_Def::Path::FILE_BMI_CALIB_CONST)) {
        ESP_LOGW(TAG, "No calibration file found. Using default offsets (0.0)");
        _offsetX = 0.0f; _offsetY = 0.0f; _offsetZ = 0.0f;
        return false;
    }

    File v_file = LittleFS.open(T2_Def::Path::FILE_BMI_CALIB_CONST, "r");
...
bool CL_T2_SensorEngine::_readRegs(uint8_t p_reg, uint8_t* p_data, uint16_t p_len) {
    ...
    digitalWrite(T2_Def::Hardware::PIN_BMI_CS_CONST, LOW);
    _spi.transfer(p_reg | 0x80);
    _spi.transfer(0x00);
    for (uint16_t i = 0; i < p_len; i++) p_data[i] = _spi.transfer(0x00);
    digitalWrite(T2_Def::Hardware::PIN_BMI_CS_CONST, HIGH);
    _spi.endTransaction();
    return true;
}
