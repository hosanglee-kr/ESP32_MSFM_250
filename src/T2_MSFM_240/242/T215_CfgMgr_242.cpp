/* ============================================================================
 * File: T215_CfgMgr_242.cpp
 * Summary: 동적 JSON 설정 관리자 구현부
 * ============================================================================
 * [로직 상태 추적 (Logic State Tracker)]
 * - 유지: LittleFS 마운트 및 /sys/runtime_cfg_242.json 경로 연동.
...
// ArduinoJson V7용 PSRAM 커스텀 할당자 (메모리 단편화 원천 차단)
...
bool CL_T2_ConfigManager::init() {
    ...
    if (!LittleFS.exists(T2_Def::Path::FILE_CFG_JSON_CONST)) {
        if (LittleFS.exists("/sys/config_242.tmp")) {
            ESP_LOGW(TAG, "Recovering config from interrupted atomic write (.tmp -> .json)");
            LittleFS.rename("/sys/config_242.tmp", T2_Def::Path::FILE_CFG_JSON_CONST);
            load();
        } else {
...
void CL_T2_ConfigManager::_loadDefaults() {
    // [교정] System 메타데이터 및 스트리밍 제어 기본값 
    strlcpy(_config.system.site_id, T2_Def::System::SITE_ID_DEF, sizeof(_config.system.site_id));
    _config.system.telemetry_hz = T2_Def::System::TELEMETRY_HZ_DEF;
    _config.system.waveform_hz = T2_Def::System::WAVEFORM_HZ_DEF;

    // [교정] 매직 넘버 제거 및 ODR 누락 복원
    _config.sensor.accel_range = T2_Def::Sensor::ACCEL_RANGE_DEF; 
    _config.sensor.accel_odr   = T2_Def::Sensor::ACCEL_ODR_DEF;
    _config.sensor.gyro_range  = T2_Def::Sensor::GYRO_RANGE_DEF;  

    _config.trigger.hold_time_ms = T2_Def::Trigger::HOLD_TIME_MS_DEF;
    _config.trigger.use_deep_sleep = T2_Def::Trigger::USE_DEEP_SLEEP_DEF;
    _config.trigger.sleep_timeout_sec = T2_Def::Trigger::SLEEP_TIMEOUT_SEC_DEF;
    _config.trigger.motion_en = false; // [복원] 인터럽트 기본값
    _config.trigger.wake_thresh_g = T2_Def::Trigger::WAKE_THRESH_G_DEF;
    _config.trigger.wake_duration = T2_Def::Trigger::WAKE_DURATION_DEF;
    _config.trigger.vib_rms_thresh = T2_Def::Decision::RULE_VIB_RMS_THRESH_DEF;
    _config.trigger.audio_rms_thresh = T2_Def::Decision::RULE_AUDIO_RMS_THRESH_DEF;

    for (uint8_t i = 0; i < T2_Def::FeatureLimit::MAX_BAND_RMS_CONST; i++) {
    ...
    _config.dsp.median_window = T2_Def::Dsp::MEDIAN_WINDOW_DEF;

    _config.dsp.fir_hpf.enabled = true;
    _config.dsp.fir_hpf.cutoff_hz = T2_Def::Dsp::FIR_HPF_CUTOFF_DEF;
    _config.dsp.fir_hpf.num_taps = T2_Def::FeatureLimit::FIR_TAPS_CONST;

    _config.dsp.fir_lpf.enabled = false;
    _config.dsp.fir_lpf.cutoff_hz = 1000.0f;
    _config.dsp.fir_lpf.num_taps = T2_Def::FeatureLimit::FIR_TAPS_CONST;
    ...
    _config.dsp.notch.target_freq_hz = T2_Def::Dsp::NOTCH_FREQ_HZ_DEF;
    _config.dsp.notch.gain = 1.0f;
    _config.dsp.notch.q_factor = T2_Def::Dsp::NOTCH_Q_FACTOR_DEF;

    _config.dsp.preemphasis_enable = true;
    _config.dsp.preemphasis_alpha = T2_Def::Dsp::PRE_EMPHASIS_ALPHA_DEF;
    ...
    _config.dsp.beamforming_gain = T2_Def::Dsp::BEAMFORMING_GAIN_DEF;
    ...
    _config.dsp.calib_eq_coeffs[T2_Def::FeatureLimit::FIR_TAPS_CONST / 2] = 1.0f; 
    ...
    _config.storage.rotation_mb = T2_Def::Storage::ROTATE_MB_DEF;
    _config.storage.rotation_min = T2_Def::Storage::ROTATE_MIN_DEF;
    _config.storage.save_raw = false;
    _config.storage.rotate_keep_max = T2_Def::Storage::ROTATE_KEEP_MAX_DEF;
    _config.storage.idle_flush_ms = T2_Def::Storage::IDLE_FLUSH_MS_DEF;
    _config.storage.pre_trigger_sec = T2_Def::Storage::PRE_TRIGGER_SEC_DEF;

    _config.wifi.mode = T2_Type::WiFiMode::AUTO_FALLBACK;
    // [교정] 문자열 상수화
    strlcpy(_config.wifi.ap_ssid, T2_Def::Net::WIFI_AP_SSID_DEF, sizeof(_config.wifi.ap_ssid));
    strlcpy(_config.wifi.ap_password, T2_Def::Net::WIFI_AP_PW_DEF, sizeof(_config.wifi.ap_password));
    strlcpy(_config.wifi.ap_ip, T2_Def::Net::WIFI_AP_IP_DEF, sizeof(_config.wifi.ap_ip));

    for (uint8_t i = 0; i < T2_Def::Net::WIFI_MULTI_MAX_CONST; i++) {
    ...
    _config.mqtt.port = T2_Def::Net::MQTT_PORT_CONST;
    strlcpy(_config.mqtt.id, T2_Def::Net::MQTT_ID_DEF, sizeof(_config.mqtt.id));
    strlcpy(_config.mqtt.password, "", sizeof(_config.mqtt.password));
    strlcpy(_config.mqtt.topic_root, T2_Def::Net::MQTT_TOPIC_DEF, sizeof(_config.mqtt.topic_root));
    _config.mqtt.qos = 1;

    _config.op_mode = 0;
    _config.watchdog_ms = T2_Def::Task::WDG_TIMEOUT_MS_CONST;
}
...
void CL_T2_ConfigManager::_applyJson(const JsonDocument& p_doc) {
    ...
    if (!v_sys.isNull()) {
        ...
        cfgSet(v_sys["telemetry_hz"], _config.system.telemetry_hz);
        cfgSet(v_sys["waveform_hz"],  _config.system.waveform_hz);
    }

    JsonObjectConst v_sensor = p_doc["sensor"];
    if (!v_sensor.isNull()) {
        cfgSet(v_sensor["accel_range"], _config.sensor.accel_range);
        cfgSet(v_sensor["accel_odr"],   _config.sensor.accel_odr); // [복원]
    ...
    if (!v_trigger.isNull()) {
        ...
        cfgSet(v_trigger["motion_en"],         _config.trigger.motion_en); // [복원]
        ...
        if (!v_bands.isNull()) {
            ...
            for (JsonObjectConst v_b : v_bands) {
                if (i >= T2_Def::FeatureLimit::MAX_BAND_RMS_CONST) break;
    ...
    if (!v_dsp.isNull()) {
        ...
        if (!v_eq.isNull()) {
            ...
            for (JsonVariantConst v_val : v_eq) { 
                if (i >= T2_Def::FeatureLimit::FIR_TAPS_CONST) break;
    ...
    if (!v_feature.isNull()) {
        ...
        if (!v_ceps.isNull()) {
            ...
            for (JsonVariantConst v_val : v_ceps) {
                if (i >= 3) break;
                _config.feature.ceps_targets[i] = v_val.as<float>();
                i++;
            }
        }
    }
    ...
    if (!v_wifi.isNull()) {
        ...
        if (!v_multi.isNull()) {
            ...
            for (JsonObjectConst v_ap : v_multi) {
                if (i >= T2_Def::Net::WIFI_MULTI_MAX_CONST) break;
...
bool CL_T2_ConfigManager::load() {
    ...
    File v_file = LittleFS.open(T2_Def::Path::FILE_CFG_JSON_CONST, "r");
    if (!v_file) {
...
bool CL_T2_ConfigManager::save() {
    ...
    // [교정] PSRAM Custom Allocator 적용 (Heap 파편화 방어)
    JsonDocument v_doc(&g_psramAlloc);

    // [복원] System
    JsonObject v_sys = v_doc["system"].to<JsonObject>();
    v_sys["site_id"] = _config.system.site_id;
    v_sys["telemetry_hz"] = _config.system.telemetry_hz;
    v_sys["waveform_hz"] = _config.system.waveform_hz;

    JsonObject v_sensor = v_doc["sensor"].to<JsonObject>();
    v_sensor["accel_range"] = _config.sensor.accel_range;
    v_sensor["accel_odr"] = _config.sensor.accel_odr; // [복원]
    ...
    v_trigger["motion_en"] = _config.trigger.motion_en; // [복원]
    ...
    // [교정] 부동소수점 정밀도 보존 헬퍼 적용 (필터 붕괴 완벽 차단)
    JsonArray v_eq = v_dsp["calib_eq_coeffs"].to<JsonArray>();
    for(uint16_t i = 0; i < T2_Def::FeatureLimit::FIR_TAPS_CONST; i++) {
        cfgAddFloatExact(v_eq, _config.dsp.calib_eq_coeffs[i]);
    }
    ...
    for(uint8_t i = 0; i < 3; i++) {
        cfgAddFloatExact(v_ceps, _config.feature.ceps_targets[i]);
    }
    ...
    File v_file = LittleFS.open("/sys/config_242.tmp", "w");
    if (!v_file) {
    ...
    LittleFS.remove(T2_Def::Path::FILE_CFG_JSON_CONST);
    LittleFS.rename("/sys/config_242.tmp", T2_Def::Path::FILE_CFG_JSON_CONST);
