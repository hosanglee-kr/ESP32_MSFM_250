/* ============================================================================
 * File: T240_Main_242.hpp (main.cpp에서 include 및 T240_init 호출)
...
void IRAM_ATTR T240_handleTriggerISR() {
    ...
    if (v_current_us - v_last_trigger_us > 200000ULL) {
        ...
        bool v_is_active = digitalRead(T2_Def::Hardware::PIN_BTN_CONTROL_CONST);
...
inline void T2_init() {
    // 1. 시리얼 통신 기동
    Serial.begin(T2_Def::Hardware::SERIAL_BAUD_CONST);
    ...
    pinMode(T2_Def::Hardware::PIN_BTN_CONTROL_CONST, INPUT_PULLDOWN);
    attachInterrupt(digitalPinToInterrupt(T2_Def::Hardware::PIN_BTN_CONTROL_CONST), T240_handleTriggerISR, CHANGE);

    // 4. [블라인드 스팟 차단] 인터럽트 활성화 직후 현재 물리 상태를 강제 동기화
    bool v_boot_trigger_state = digitalRead(T2_Def::Hardware::PIN_BTN_CONTROL_CONST);
