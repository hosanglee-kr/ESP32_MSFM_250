...
bool CL_T2_FeatureExtractor::init() {
    ...
    _fftWorkAudio = (float*)heap_caps_aligned_alloc(16, T2_Def::System::FFT_SIZE_AUDIO_CONST * 2 * sizeof(float), MALLOC_CAP_SPIRAM);
    _powerAudio = (float*)heap_caps_aligned_alloc(16, BINS_PADDED * sizeof(float), MALLOC_CAP_SPIRAM);
    _windowAudio = (float*)heap_caps_aligned_alloc(16, T2_Def::System::FFT_SIZE_AUDIO_CONST * sizeof(float), MALLOC_CAP_SPIRAM);

    if (!_melBankFlat || !_dctMatrixFlat || !_fftWorkAudio || !_powerAudio || !_windowAudio) return false;

    dsps_wind_hann_f32(_windowAudio, T2_Def::System::FFT_SIZE_AUDIO_CONST);
    dsps_wind_hann_f32(_windowVib, T2_Def::System::FFT_SIZE_VIB_CONST);
...
void CL_T2_FeatureExtractor::extractVibration(const float* p_vibX, const float* p_vibY, const float* p_vibZ,
    ...
    float v_scale = (1.0f / (float)T2_Def::System::FFT_SIZE_VIB_CONST) * 2.0f * 2.666f;
    ...
    for (uint8_t v_axis = 0; v_axis < T2_Def::System::VIB_AXIS_CONST; v_axis++) {
        ...
        // 4. FFT 및 Peak 추출
        dsps_fft2r_fc32(_fftWorkVib[v_axis], T2_Def::System::FFT_SIZE_VIB_CONST);
        dsps_bit_rev2r_fc32(_fftWorkVib[v_axis], T2_Def::System::FFT_SIZE_VIB_CONST);

        for (uint16_t i = 0; i < T2_Def::System::FFT_SIZE_VIB_CONST / 2; i++) {
...
void CL_T2_FeatureExtractor::_computeVibPeaks(uint8_t p_axis, T2_Type::UnifiedFeatureSlot& p_slot) {
    float v_binRes = (float)T2_Def::System::RATE_VIB_CONST / T2_Def::System::FFT_SIZE_VIB_CONST;
    float v_maxPower = 0.0f;
    float v_peakHz = 0.0f;

    // 단순 Max Peak (Vib은 대표 주파수 1개만 우선 슬롯에 저장)
    for (uint16_t i = 1; i < T2_Def::System::FFT_SIZE_VIB_CONST / 2; i++) {
...
void CL_T2_FeatureExtractor::extractAudio(const float* p_audioBeam, uint32_t p_len, T2_Type::UnifiedFeatureSlot& p_slot) {
    ...
    dsps_fft2r_fc32(_fftWorkAudio, T2_Def::System::FFT_SIZE_AUDIO_CONST);
    dsps_bit_rev2r_fc32(_fftWorkAudio, T2_Def::System::FFT_SIZE_AUDIO_CONST);

    float v_scale = (1.0f / (float)T2_Def::System::FFT_SIZE_AUDIO_CONST) * 2.0f * 2.666f;
    uint16_t v_bins = (T2_Def::System::FFT_SIZE_AUDIO_CONST / 2) + 1;
...
void CL_T2_FeatureExtractor::_computeAudioMfcc39(T2_Type::UnifiedFeatureSlot& p_slot) {
    ...
    for (uint16_t m = 0; m < T2_Def::System::MEL_BANDS_CONST; m++)
    ...
    memcpy(p_slot.mfcc, v_dctOut, T2_Def::System::MFCC_COEFFS_CONST * sizeof(float));
...
void CL_T2_FeatureExtractor::_applyTemporalDerivatives(T2_Type::UnifiedFeatureSlot& p_slot) {
    ...
    uint16_t v_dim = T2_Def::System::MFCC_COEFFS_CONST; // 13
