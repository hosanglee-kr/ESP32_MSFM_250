 ...
 * @brief 시스템 전역 설정값 보존 및 LittleFS 동적 연동 관리 클래스
 */
class CL_T2_ConfigManager {
private:
    T2_Type::ST_DynamicConfig_t _config;                 ///< 4-Tier 통합 설정 단일 진실 공급원 (SSOT)
...
public:
    /**
     * @brief 싱글톤 인스턴스 반환
     * @return CL_T2_ConfigManager& 싱글톤 인스턴스 참조
     */
    ...
    // 데이터 접근 및 조작 메서드
    T2_Type::ST_DynamicConfig_t getConfig();             // 현재 시스템 설정 구조체 스냅샷 복사본 반환 (Thread-Safe)
    bool updateConfig(const T2_Type::ST_DynamicConfig_t& p_newConfig); // 전체 구조체 덮어쓰기 및 플래시 즉시 저장
