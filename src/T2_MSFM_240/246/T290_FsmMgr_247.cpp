...
bool CL_T2_FsmManager::init() {
    ...
    if (v_cfg.audio.enable) {
        _capBufL = (float*)heap_caps_aligned_alloc(16, v_fSizeAudio, MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
        _prcBufL = (float*)heap_caps_aligned_alloc(16, v_fSizeAudio, MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
        _capBufR = (float*)heap_caps_aligned_alloc(16, v_fSizeAudio, MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
        _prcBufR = (float*)heap_caps_aligned_alloc(16, v_fSizeAudio, MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
    }
    
    // TODO : accel , gyro 각각 할당
    if (v_cfg.accel.enable || v_cfg.gyro.enable) {
        _accBufX = (float*)heap_caps_aligned_alloc(16, v_fSizeVib, MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
        _accBufY = (float*)heap_caps_aligned_alloc(16, v_fSizeVib, MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
        _accBufZ = (float*)heap_caps_aligned_alloc(16, v_fSizeVib, MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
        _gyrBufX = (float*)heap_caps_aligned_alloc(16, v_fSizeVib, MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
        _gyrBufY = (float*)heap_caps_aligned_alloc(16, v_fSizeVib, MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
        _gyrBufZ = (float*)heap_caps_aligned_alloc(16, v_fSizeVib, MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
    }
...
void CL_T2_FsmManager::_imuAcqTask(void* p_param) {
    ...
    while (v_this->_state != T2_Type::EM_SystemState_t::INIT) {
        ...
        if (v_this->_state != T2_Type::EM_SystemState_t::MAINTENANCE) {
            ...
            uint8_t int_stat_1 = v_this->_sensor._readRegSingle(T2_Def::Imu::Hardware::REG_INT_STATUS_1_ADDR);
            if (int_stat_1 & T2_Def::Imu::Hardware::REG_FIFO_WTM_STATUS_BIT) {
...
void CL_T2_FsmManager::_vibProcessTask(void* p_param) {
    ...
    float* acc_proc_x = (float*)heap_caps_malloc(T2_Def::Accel::Sensor::FFT_SIZE_MAX * sizeof(float), MALLOC_CAP_INTERNAL);
    float* acc_proc_y = (float*)heap_caps_malloc(T2_Def::Accel::Sensor::FFT_SIZE_MAX * sizeof(float), MALLOC_CAP_INTERNAL);
    float* acc_proc_z = (float*)heap_caps_malloc(T2_Def::Accel::Sensor::FFT_SIZE_MAX * sizeof(float), MALLOC_CAP_INTERNAL);
    float* gyr_proc_x = (float*)heap_caps_malloc(T2_Def::Gyro::Sensor::FFT_SIZE_MAX * sizeof(float), MALLOC_CAP_INTERNAL);
    float* gyr_proc_y = (float*)heap_caps_malloc(T2_Def::Gyro::Sensor::FFT_SIZE_MAX * sizeof(float), MALLOC_CAP_INTERNAL);
    float* gyr_proc_z = (float*)heap_caps_malloc(T2_Def::Gyro::Sensor::FFT_SIZE_MAX * sizeof(float), MALLOC_CAP_INTERNAL);
    ...
    while (v_this->_state != T2_Type::EM_SystemState_t::INIT) {
        if (v_this->_state == T2_Type::EM_SystemState_t::MONITORING ||
            ...
            if (v_this->_sensor.getAccumulatedAccelCount() >= T2_Def::Accel::Sensor::FFT_SIZE_MAX) {
                ...
                uint8_t write_idx = v_this->_sharedCtx->vib_idx.load(std::memory_order_relaxed) ^ 1;
                auto& slot = v_this->_sharedCtx->vib_slots[write_idx];
                memset(&slot, 0, sizeof(T2_Type::ST_FeatureSlot_Vib_t)); // 패딩 영역 0 초기화

                // 헤더 조립
                slot.header.ts = (uint64_t)(esp_timer_get_time() / 1000);
                slot.header.uptime = millis();
                slot.header.temp = v_this->_sensor.readTemperatureSensor();
                slot.header.payload_type = (uint8_t)T2_Type::EM_DataPayloadType_t::VIB_ONLY;
                slot.header.accel_mask = v_cfg.accel.axis_mask;
                slot.header.gyro_mask  = v_cfg.gyro.axis_mask;

                // 2. 가속도/자이로 원시 데이터 인출 (Zero-Copy 링버퍼 활용)
                v_this->_sensor.getAccumulatedAccel(acc_proc_x, acc_proc_y, acc_proc_z, T2_Def::Accel::Sensor::FFT_SIZE_MAX);
                v_this->_sensor.getAccumulatedGyro(gyr_proc_x, gyr_proc_y, gyr_proc_z, T2_Def::Gyro::Sensor::FFT_SIZE_MAX);

                // 3. DSP 가공 및 특징 추출
                if (v_cfg.accel.enable) {
                    v_this->_dsp.processAccel(acc_proc_x, acc_proc_y, acc_proc_z, acc_proc_x, acc_proc_y, acc_proc_z, T2_Def::Accel::Sensor::FFT_SIZE_MAX, v_cfg.accel);
                    v_this->_extractor.extractAccel(acc_proc_x, acc_proc_y, acc_proc_z, T2_Def::Accel::Sensor::FFT_SIZE_MAX, v_cfg.accel.sample_rate, slot, v_cfg.accel);
                }

                if (v_cfg.gyro.enable) {
                    v_this->_dsp.processGyro(gyr_proc_x, gyr_proc_y, gyr_proc_z, gyr_proc_x, gyr_proc_y, gyr_proc_z, T2_Def::Gyro::Sensor::FFT_SIZE_MAX, v_cfg.gyro);
                    v_this->_extractor.extractGyro(gyr_proc_x, gyr_proc_y, gyr_proc_z, T2_Def::Gyro::Sensor::FFT_SIZE_MAX, v_cfg.gyro.sample_rate, slot, v_cfg.gyro);
                }

                // 글로벌 스트리밍 버퍼에 복사 스냅샷 생성 (웹소켓 전송용 동기화 캐시 스페이스 정합)
                if (v_this->_accBufX) memcpy(v_this->_accBufX, acc_proc_x, T2_Def::Accel::Sensor::FFT_SIZE_MAX * sizeof(float));
                if (v_this->_accBufY) memcpy(v_this->_accBufY, acc_proc_y, T2_Def::Accel::Sensor::FFT_SIZE_MAX * sizeof(float));
                if (v_this->_accBufZ) memcpy(v_this->_accBufZ, acc_proc_z, T2_Def::Accel::Sensor::FFT_SIZE_MAX * sizeof(float));
                if (v_this->_gyrBufX) memcpy(v_this->_gyrBufX, gyr_proc_x, T2_Def::Gyro::Sensor::FFT_SIZE_MAX * sizeof(float));
                if (v_this->_gyrBufY) memcpy(v_this->_gyrBufY, gyr_proc_y, T2_Def::Gyro::Sensor::FFT_SIZE_MAX * sizeof(float));
                if (v_this->_gyrBufZ) memcpy(v_this->_gyrBufZ, gyr_proc_z, T2_Def::Gyro::Sensor::FFT_SIZE_MAX * sizeof(float));

                // 4. 원자적 스왑: 오디오 태스크가 이제 무등급으로 읽어갈 수 있게 배포 장벽 해제
                v_this->_sharedCtx->vib_idx.store(write_idx, std::memory_order_release);

                // [Phase 4 완료] 진동 특징량 구조체 및 복사된 원시 정적 버퍼를 스토리지 비동기 엔진에 인입
                T2_Type::ST_Raw_Accel_t v_tmpAcc;
                v_tmpAcc.ts = slot.header.ts;
                v_tmpAcc.sample_rate = v_cfg.accel.sample_rate;
                v_tmpAcc.active_mask = slot.header.accel_mask;
                memcpy(v_tmpAcc.data[0], acc_proc_x, T2_Def::Accel::Sensor::FFT_SIZE_MAX * sizeof(float));
                memcpy(v_tmpAcc.data[1], acc_proc_y, T2_Def::Accel::Sensor::FFT_SIZE_MAX * sizeof(float));
                memcpy(v_tmpAcc.data[2], acc_proc_z, T2_Def::Accel::Sensor::FFT_SIZE_MAX * sizeof(float));

                T2_Type::ST_Raw_Gyro_t v_tmpGyr;
                v_tmpGyr.ts = slot.header.ts;
                v_tmpGyr.sample_rate = v_cfg.gyro.sample_rate;
                v_tmpGyr.active_mask = slot.header.gyro_mask;
                memcpy(v_tmpGyr.data[0], gyr_proc_x, T2_Def::Gyro::Sensor::FFT_SIZE_MAX * sizeof(float));
                memcpy(v_tmpGyr.data[1], gyr_proc_y, T2_Def::Gyro::Sensor::FFT_SIZE_MAX * sizeof(float));
                memcpy(v_tmpGyr.data[2], gyr_proc_z, T2_Def::Gyro::Sensor::FFT_SIZE_MAX * sizeof(float));

                v_this->_storage.pushVibFrame(&slot, &v_tmpAcc, &v_tmpGyr);
            }
        }
    ...
    // 종료 시 리소스 반환
    if (acc_proc_x) heap_caps_free(acc_proc_x);
    if (acc_proc_y) heap_caps_free(acc_proc_y);
    if (acc_proc_z) heap_caps_free(acc_proc_z);
    if (gyr_proc_x) heap_caps_free(gyr_proc_x);
    if (gyr_proc_y) heap_caps_free(gyr_proc_y);
    if (gyr_proc_z) heap_caps_free(gyr_proc_z);
...
void CL_T2_FsmManager::_audioProcessTask(void* p_param) {
    ...
    float* p_outL = v_this->_prcBufL;
    float* p_outR = v_this->_prcBufR;
    float* p_rawL = v_this->_capBufL;
    float* p_rawR = v_this->_capBufR;
    ...
    while (v_this->_state != T2_Type::EM_SystemState_t::INIT) {
        if (v_this->_state == T2_Type::EM_SystemState_t::MONITORING ||
            ...
            uint32_t samples = v_this->_sensor.readAudioChunk(p_rawL, p_rawR, T2_Def::Audio::Sensor::FFT_SIZE_MAX);

            if (samples > 0) {
                // 더블 버퍼링: 쓰기 인덱스 결정
                uint8_t write_idx = v_this->_sharedCtx->aud_idx.load(std::memory_order_relaxed) ^ 1;
                auto& aud_slot = v_this->_sharedCtx->aud_slots[write_idx];
                memset(&aud_slot, 0, sizeof(T2_Type::ST_FeatureSlot_Aud_t));

                // 헤더 조립
                aud_slot.header.ts = (uint64_t)(esp_timer_get_time() / 1000);
                aud_slot.header.uptime = millis();
                aud_slot.header.payload_type = (uint8_t)T2_Type::EM_DataPayloadType_t::AUDIO_ONLY;
                aud_slot.header.audio_mask = v_cfg.audio.channel_mask;

                // 2. 오디오 DSP 가공 및 특징 추출
                v_this->_dsp.processAudio(p_rawL, p_rawR, p_outL, p_outR, samples, v_cfg.audio);
                v_this->_extractor.extractAudio(p_outL, p_outR, samples, v_cfg.audio.sample_rate, aud_slot, v_cfg.audio);

                // 3. 원자적 스왑 (오디오 슬롯 갱신)
                v_this->_sharedCtx->aud_idx.store(write_idx, std::memory_order_release);

                // 4. Late-Sync: 가장 최신 진동 슬롯을 Lock-free 인출
                uint8_t vib_idx = v_this->_sharedCtx->vib_idx.load(std::memory_order_acquire);
                const auto& vib = v_this->_sharedCtx->vib_slots[vib_idx];

                // 텐서 조립 (진동 데이터 초기화가 완료된 시점부터)
                if (vib.header.ts > 0) {
                    memcpy(v_this->_flatTensor, aud_slot.mfcc, 78 * sizeof(float));
                    memcpy(v_this->_flatTensor + 78, vib.mfcc, 234 * sizeof(float));
                ...
                if (v_this->_state == T2_Type::EM_SystemState_t::MONITORING || v_this->_state == T2_Type::EM_SystemState_t::RECORDING) {
                    T2_Type::EM_DetectionResult_t v_res = v_this->_trigger.runDiagnostic(aud_slot, vib, v_cfg);
                    v_this->_handleTriggerResult(v_res, aud_slot, vib);
                } else if (v_this->_state == T2_Type::EM_SystemState_t::CALIBRATING) {
                    // 캘리브레이션 세션 활성화 상태 시 무조건 밀어넣기 처리
                    T2_Type::ST_Raw_Audio_t v_tmpAud;
                    v_tmpAud.ts = aud_slot.header.ts;
                    v_tmpAud.sample_rate = v_cfg.audio.sample_rate;
                    v_tmpAud.active_mask = aud_slot.header.audio_mask;
                    memcpy(v_tmpAud.data[0], p_rawL, T2_Def::Audio::Sensor::FFT_SIZE_MAX * sizeof(float));
                    memcpy(v_tmpAud.data[1], p_rawR, T2_Def::Audio::Sensor::FFT_SIZE_MAX * sizeof(float));
                    v_this->_storage.pushAudioFrame(&aud_slot, &v_tmpAud);
                }

                // [Phase 4 완료] 실시간 스트리밍 다중 파형 매핑 호출 전달 정합 완료
                v_this->_broadcastStreams(aud_slot, vib, p_rawL, p_rawR);
            }
        }
...
void CL_T2_FsmManager::_broadcastStreams(const T2_Type::ST_FeatureSlot_Aud_t& p_audSlot,
    ...
    if (_accTele >= 100) {
        ...
        // 가속도 특징량 매핑 복원
        for (int i = 0; i < 3; i++) {
            _pktTele->accel_rms[i]           = p_vibSlot.accel.rms[i];
            _pktTele->accel_centroid[i]      = p_vibSlot.accel.centroid[i];
            _pktTele->accel_kurtosis[i]      = p_vibSlot.accel.kurt[i];
            _pktTele->accel_crest_factor[i]  = p_vibSlot.accel.crest[i];
            _pktTele->accel_sta_lta_ratio[i] = p_vibSlot.accel.sta_lta[i];
            _pktTele->accel_skewness[i]      = p_vibSlot.accel.skew[i];
            _pktTele->accel_std[i]           = p_vibSlot.accel.std[i];
            memcpy(_pktTele->accel_band_energy[i], p_vibSlot.accel.band_energy[i], sizeof(_pktTele->accel_band_energy[i]));
        }

        // 자이로 특징량 매핑 복원
        for (int i = 0; i < 3; i++) {
            _pktTele->gyro_rms[i]           = p_vibSlot.gyro.rms[i];
            _pktTele->gyro_centroid[i]      = p_vibSlot.gyro.centroid[i];
            _pktTele->gyro_kurtosis[i]      = p_vibSlot.gyro.kurt[i];
            _pktTele->gyro_crest_factor[i]  = p_vibSlot.gyro.crest[i];
            _pktTele->gyro_sta_lta_ratio[i] = p_vibSlot.gyro.sta_lta[i];
            _pktTele->gyro_skewness[i]      = p_vibSlot.gyro.skew[i];
            _pktTele->gyro_std[i]           = p_vibSlot.gyro.std[i];
            _pktTele->gyro_drift_est[i]     = p_vibSlot.gyro.drift_est[i];
            memcpy(_pktTele->gyro_band_energy[i], p_vibSlot.gyro.band_energy[i], sizeof(_pktTele->gyro_band_energy[i]));
        }

        // 오디오 특징량 매핑 복원
        for (int ch = 0; ch < 2; ch++) {
            _pktTele->audio_rms[ch]                = p_audSlot.audio.ch[ch].rms;
            _pktTele->audio_energy[ch]             = p_audSlot.audio.ch[ch].energy;
            _pktTele->audio_kurtosis[ch]           = p_audSlot.audio.ch[ch].kurt;
            _pktTele->audio_crest_factor[ch]       = p_audSlot.audio.ch[ch].crest;
            _pktTele->audio_sta_lta_ratio[ch]      = p_audSlot.audio.ch[ch].sta_lta;
            _pktTele->audio_skewness[ch]           = p_audSlot.audio.ch[ch].skew;
            _pktTele->audio_spectral_centroid[ch]  = p_audSlot.audio.ch[ch].centroid;
            memcpy(_pktTele->audio_band_energy[ch], p_audSlot.audio.ch[ch].band_energy, sizeof(_pktTele->audio_band_energy[ch]));

            for (int p = 0; p < T2_Def::Audio::FeatureLimit::TOP_PEAKS_MAX; p++) {
                _pktTele->audio_peak_freqs[ch][p]  = p_audSlot.audio.ch[ch].top_peaks[p].freq;
                _pktTele->audio_peak_amps[ch][p]   = p_audSlot.audio.ch[ch].top_peaks[p].amp;
            }

            for (int c = 0; c < T2_Def::Audio::FeatureLimit::CEPS_TARGET_MAX; c++) {
                _pktTele->audio_cpsr_max[ch][c]    = p_audSlot.audio.ch[ch].cpsr_max[c];
                _pktTele->audio_cpsr_mxrms[ch][c]  = p_audSlot.audio.ch[ch].cpsr_mxr[c];
            }
        }

        _pktTele->audio_coh = p_audSlot.audio.coh;
        _pktTele->audio_ipd = p_audSlot.audio.ipd;
        _pktTele->accel_ts  = p_vibSlot.header.ts;
        _pktTele->gyro_ts   = p_vibSlot.header.ts;
        _pktTele->audio_ts  = p_audSlot.header.ts;

        // 최종 직렬화 활성 평탄화 융합 텐서 배치
        memcpy(_pktTele->mfcc, _flatTensor, sizeof(_pktTele->mfcc));
    ...
    if (_accWave >= 100) {
        ...
        if (v_cfg.accel.enable && _accBufX) {
            for (int i = 0; i < 3; i++) {
                if (v_cfg.accel.axis_mask & (1 << i)) {
                    ...
                    float* v_srcBuf = (i == 0) ? _accBufX : (i == 1) ? _accBufY : _accBufZ;
...
void CL_T2_FsmManager::_handleTriggerResult(T2_Type::EM_DetectionResult_t p_res,
    ...
    if (p_res != T2_Type::EM_DetectionResult_t::PASS) {
        ...
        if (_capBufL) memcpy(v_tmpAud.data[0], _capBufL, v_audBlockSize);
        if (_capBufR) memcpy(v_tmpAud.data[1], _capBufR, v_audBlockSize);
        ...
        if (_accBufX) memcpy(v_tmpAcc.data[0], _accBufX, v_vibBlockSize);
        if (_accBufY) memcpy(v_tmpAcc.data[1], _accBufY, v_vibBlockSize);
        if (_accBufZ) memcpy(v_tmpAcc.data[2], _accBufZ, v_vibBlockSize);
        ...
        if (_gyrBufX) memcpy(v_tmpGyr.data[0], _gyrBufX, v_vibBlockSize);
        if (_gyrBufY) memcpy(v_tmpGyr.data[1], _gyrBufY, v_vibBlockSize);
        if (_gyrBufZ) memcpy(v_tmpGyr.data[2], _gyrBufZ, v_vibBlockSize);
