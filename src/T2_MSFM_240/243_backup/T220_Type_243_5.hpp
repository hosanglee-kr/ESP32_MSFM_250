/* ============================================================================
 * File: T220_Type_243_5.hpp
 * Summary: T240 4-Tier 데이터 타입, 설정 구조체, 패킷 포맷 통합 정의 (완성체)
 * ============================================================================
 ...
 * 5. [MLOps 추적성]: FileHeader에 설정 덤프 삽입 및 패킷 내 active_axes 정보 명시.
 * 6. [런타임 격리]: JSON 동적 설정과 FSM 메모리 런타임 컨텍스트(RuntimeCtx)의 분리.
 * ========================================================================== */

#pragma once

#include "T210_Def_243_5.hpp"
#include <cstdint>
...
namespace T2_Type {
    ...
    enum class OpMode : uint8_t { // [복원] 시스템 구동 모드
        MANUAL = 0, AUTO, SCHEDULE
    };
    ...
    // ========================================================================
    // [PART 2] 동적 설정 구조체 (4-Tier 도메인 분리 & 익명구조체 전면 금지)
    // ========================================================================
    ...
    struct ST_Global_Mqtt { 
        ...
        char pw[64]; 
        char topic_root[64]; 
        char lwt_topic[64];  // [복원] Last Will and Testament
        uint8_t qos; 
        uint8_t proto_ver; 
    };
    ...
    struct ST_Vib_Sensor { 
        uint8_t axis;         // [복원] 타겟 분석 축 선택
        uint8_t axis_count;   // [복원] 1축 vs 3축 분석 제어
        uint8_t accel_range; 
        uint8_t accel_odr; 
        uint8_t gyro_range; 
    };
    ...
    // ------------------------------------------------------------------------
    // 런타임 제어 컨텍스트 (비휘발성 설정이 아닌 FSM 메모리 전용) [복원]
    // ------------------------------------------------------------------------
    ...
    struct ST_Slot_Header {
        ...
        uint8_t flags; int8_t temp; uint8_t trial; uint8_t src; 
        uint8_t active_axes; // [복원] 활성화된 축 개수 추적용
        uint8_t _res[11];    // 1바이트 축소하여 총 32 Bytes 정렬 완벽 유지
    };
    
    // [Block 2] 진동 특화 지표 (80 Bytes) 
    struct ST_Slot_Vib {
        float rms[3]; float peak_f[3]; float centroid[3]; float kurt; float crest; float sta_lta; float skew; float std; 
        float cal_off[3]; float _pad[3]; 
    };
    ...
    struct ST_Slot_Audio {
        ...
        float cpsr_max[3]; float cpsr_mxr[3]; float _pad[2]; 
    };
    ...
    // + 브라우저가 축 개수(active_axes)를 알 수 있도록 패킷 헤더에 정보 임베딩
    struct PktTelemetry {
        WsHeader header; 
        uint8_t  sys_state; 
        ...
        uint8_t  active_axes; // [복원] 패킷 단에서도 축 추적 가능
        uint8_t  _pad[3];     // 정렬을 위한 3바이트 패딩 (8바이트 블록 완성)
