...
bool CL_T2_FeatureExtractor::init() {
    ...
    _noiseProfile = (float*)heap_caps_aligned_alloc(16, BINS_PADDED * sizeof(float), MALLOC_CAP_SPIRAM);

    // [2] SRAM/PSRAM 혼합 할당 (DRAM 오버플로우 방지)
    ...
    _mfccHistory   = (float (*)[T2_Def::Shared::FeatureLimit::DELTA_HISTORY_MAX][MFCC_PADDED])heap_caps_aligned_alloc(16, sizeof(float) * 4 * T2_Def::Shared::FeatureLimit::DELTA_HISTORY_MAX * MFCC_PADDED, MALLOC_CAP_SPIRAM);
    _deltaHistory  = (float (*)[T2_Def::Shared::FeatureLimit::DELTA_HISTORY_MAX][MFCC_PADDED])heap_caps_aligned_alloc(16, sizeof(float) * 4 * T2_Def::Shared::FeatureLimit::DELTA_HISTORY_MAX * MFCC_PADDED, MALLOC_CAP_SPIRAM);
    ...
    memset(_mfccHistory, 0, sizeof(float) * 4 * T2_Def::Shared::FeatureLimit::DELTA_HISTORY_MAX * MFCC_PADDED);
    memset(_deltaHistory, 0, sizeof(float) * 4 * T2_Def::Shared::FeatureLimit::DELTA_HISTORY_MAX * MFCC_PADDED);

    // [3] 초기화 및 필터뱅크 생성
    memset(_melBankFlat, 0, BINS_PADDED * MEL_PADDED * sizeof(float));
    memset(_dctMatrixFlat, 0, MEL_PADDED * MFCC_PADDED * sizeof(float));
    memset(_noiseProfile, 0, BINS_PADDED * sizeof(float));
...
void CL_T2_FeatureExtractor::extractVibration(const float* p_vibX, const float* p_vibY, const float* p_vibZ, uint32_t p_len, uint32_t p_sampleRate, T2_Type::UnifiedFeatureSlot& p_slot, const T2_Type::ST_Vib_Feature& p_featCfg, const T2_Type::ST_Vib_Trigger& p_trigCfg, uint8_t p_axisCount) {
    ...
    float v_maxBandEnergy[T2_Def::Shared::FeatureLimit::BAND_RMS_MAX] = {0};

    int v_loops = (p_axisCount > 3) ? 3 : p_axisCount;
    for (int i = 0; i < v_loops; i++) {
        if (!v_in[i]) continue;

        // [Step 1] 통계 지표 추출 (RMS, Kurtosis, Crest Factor, Skewness, STD)
        _computeVibStats(v_in[i], p_len, p_slot.vib.rms[i], p_slot.vib.kurt, p_slot.vib.crest, p_slot.vib.skew, p_slot.vib.std);
        ...
        // [Step 3] 밴드 에너지 추출 및 MFCC 추출 (v234 복구)
        ...
        for (int b = 0; b < T2_Def::Shared::FeatureLimit::BAND_RMS_MAX; b++) {
            if (v_currentBands[b] > v_maxBandEnergy[b]) v_maxBandEnergy[b] = v_currentBands[b];
        }
    ...
    memcpy(p_slot.vib.band_energy, v_maxBandEnergy, sizeof(v_maxBandEnergy));
}

void CL_T2_FeatureExtractor::extractAudio(const float* p_audioBeam, uint32_t p_len, uint32_t p_sampleRate,
                                          T2_Type::UnifiedFeatureSlot& p_slot, const T2_Type::ST_Audio_Feature& p_featCfg, const T2_Type::ST_Audio_Trigger& p_trigCfg) {

    // [Step 1] 통계 특징량
    float v_rms, v_kurt, v_crest, v_skew, v_std;
    _computeVibStats(p_audioBeam, p_len, v_rms, v_kurt, v_crest, v_skew, v_std);
    p_slot.audio.rms   = v_rms;
    p_slot.audio.kurt  = v_kurt;
    p_slot.audio.crest = v_crest;
    p_slot.audio.skew  = v_skew;

    // [Step 2] FFT 및 Power Spectrum
    dsps_wind_hann_f32(_windowAudio, p_len);
    for (uint32_t i = 0; i < p_len; i++) {
        _fftWorkAudio[i * 2] = p_audioBeam[i] * _windowAudio[i];
        _fftWorkAudio[i * 2 + 1] = 0;
    }
    dsps_fft2r_fc32(_fftWorkAudio, p_len);
    dsps_bit_rev2r_fc32(_fftWorkAudio, p_len);

    uint32_t v_bins = p_len / 2 + 1;
    for (uint32_t i = 0; i < v_bins; i++) {
        float re = _fftWorkAudio[i * 2];
        float im = _fftWorkAudio[i * 2 + 1];
        _powerAudio[i] = (re * re + im * im) / p_len;
    }

    // [v013] 적응형 노이즈 프로필 학습 및 감산
    if (_isLearning) {
    ...
    // Spectral Centroid
    p_slot.audio.centroid = _computeSpectralCentroid(_powerAudio, v_bins, (float)p_sampleRate);

    // Top Peaks
    _extractTopPeaks(_powerAudio, v_bins, (float)p_sampleRate, p_slot.audio.top_peaks, T2_Def::Shared::FeatureLimit::TOP_PEAKS_MAX, p_featCfg.peak_amp_min, p_featCfg.peak_freq_gap_min);

    // [Step 3] 밴드 에너지 추출 (p_trigCfg 기반)
    _computeBandEnergies(_powerAudio, v_bins, (float)p_sampleRate, p_trigCfg.active_band_count, p_trigCfg.band_en, p_trigCfg.band_start, p_trigCfg.band_end, p_slot.audio.band_energy);

    // [Step 4] MFCC 추출 (채널 3: Audio)
    _computeMfcc(_powerAudio, p_slot.tensor.mfcc, 3);
}
...
void CL_T2_FeatureExtractor::resetNoiseProfile() {
    if (_noiseProfile) memset(_noiseProfile, 0, BINS_PADDED * sizeof(float));
    _learnedFrames = 0;
}
