...
T2_Type::DetectionResult CL_T2_TriggerEngine::_checkVibRules(const T2_Type::ST_Slot_Vib& p_vib, const T2_Type::DynamicConfig& p_cfg) {
    // 1. 축별 RMS 검사 (accel_axis_count만큼)
    for (int i = 0; i < p_cfg.vib_sensor.accel_axis_count; i++) {
        if (p_vib.rms[i] > p_cfg.vib_trig.rms_thresh) return T2_Type::DetectionResult::RULE_VIB_NG;
    }

    // 2. 통계 지표 검사 (첨도, 파고율)
    if (p_vib.kurt > p_cfg.vib_trig.kurt_ng_thresh) return T2_Type::DetectionResult::RULE_VIB_NG;
    if (p_vib.crest > p_cfg.vib_trig.crest_ng_thresh) return T2_Type::DetectionResult::RULE_VIB_NG;

    // 3. 밴드 에너지 검사
    for (int b = 0; b < p_cfg.vib_trig.active_band_count; b++) {
        if (p_cfg.vib_trig.band_en[b] && (p_vib.band_energy[b] > p_cfg.vib_trig.band_thresh[b])) {
...
T2_Type::DetectionResult CL_T2_TriggerEngine::_checkAudioRules(const T2_Type::ST_Slot_Audio& p_aud, const T2_Type::DynamicConfig& p_cfg) {
    // 1. 전체 RMS 검사
    if (p_aud.rms > p_cfg.aud_trig.rms_thresh) return T2_Type::DetectionResult::RULE_AUDIO_NG;

    // 2. 통계 지표 검사
    if (p_aud.kurt > p_cfg.aud_trig.kurt_ng_thresh) return T2_Type::DetectionResult::RULE_AUDIO_NG;
    if (p_aud.crest > p_cfg.aud_trig.crest_ng_thresh) return T2_Type::DetectionResult::RULE_AUDIO_NG;
    if (p_aud.skew > p_cfg.aud_trig.skew_ng_thresh) return T2_Type::DetectionResult::RULE_AUDIO_NG;

    // 3. 밴드 에너지 검사 (Air Leak 등)
    for (int b = 0; b < p_cfg.aud_trig.active_band_count; b++) {
        if (p_cfg.aud_trig.band_en[b] && (p_aud.band_energy[b] > p_cfg.aud_trig.band_thresh[b])) {
