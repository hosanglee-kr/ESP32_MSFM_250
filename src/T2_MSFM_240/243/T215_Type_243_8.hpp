...
struct ST_Vib_Trigger {
	...
	float	 rms_thresh;                                                                        // 소프트웨어 레벨에서 진동 파이프라인 전체 대역의 에너지를 평가하는 광대역 RMS 트리거 임계치
	float	 kurt_ng_thresh;                                                                    // [신규] 진동 파형 첨도(Kurtosis) 불량 판정 임계치 (5.0 부근 권장)
	float	 crest_ng_thresh;                                                                   // [신규] 진동 파고율(Crest Factor) 불량 판정 임계치
	float	 skew_ng_thresh;                                                                    // [신규] 진동 비대칭성(Skewness) 불량 판정 임계치
	...
	float	 band_thresh[T2_Def::Shared::FeatureLimit::BAND_RMS_MAX];                           // 해당 주파수 대역 내 파워 스펙트럼 밀도가 이 임계치를 초과하면 트리거 발동
};
...
struct ST_Audio_Trigger {
	float	rms_thresh;                                                                         // 전체 대역 소음 에너지 레벨을 평가하는 광대역 RMS 트리거 임계치
	float	kurt_ng_thresh;                                                                     // [신규] 소음 파형 첨도(Kurtosis) 불량 판정 임계치
	float	crest_ng_thresh;                                                                    // [시너지] 소음 파형의 충격성(Crest Factor) 불량 판정 임계치 (돌발 파열음/충돌음 감지)
	float	skew_ng_thresh;                                                                     // [시너지] 소음 파형의 비대칭성(Skewness) 불량 판정 임계치 (밸브 마찰/기계적 비대칭 마모 감지)
	...
	float	band_thresh[T2_Def::Shared::FeatureLimit::BAND_RMS_MAX];                            // 해당 소음 대역의 에너지가 이 임계치를 초과하면 불량(NG)으로 판정 및 트리거 발동
};
...
struct ST_Audio_Calib {
	...
	alignas(16) float eq_coeffs[T2_Def::Shared::FeatureLimit::FIR_TAPS_MAX];                    // 런타임에 곱해질 주파수 응답 평탄화 FIR 필터 계수 배열 (SIMD 최적화 강제 정렬)
};
...
struct ST_Slot_Vib {
	...
	float kurt;                                                                                 // 첨도 (Kurtosis) - 베어링, 기어의 표면 찍힘이나 깨짐 등 '충격성 결함' 감지에 탁월한 지표
	float crest;                                                                                // 파고율 (Crest Factor) - RMS 대비 최고 피크의 비율로, 파고가 뾰족한 순간적 충격 감지 지표
	float sta_lta;                                                                              // 단기/장기 평균 비율 - 급격한 진동 밸런스 붕괴(이벤트 발생)를 감지하는 지진계 알고리즘 지표
	float skew;                                                                                 // 왜도 (Skewness) - 파형의 비대칭성 지표. 회전체의 비대칭 마모나 축 정렬(Alignment) 불량 감지
	float std;                                                                                  // 표준편차 (Standard Deviation) - 신호의 분산 정도를 나타내는 진동의 전반적인 떨림 강도
	float cal_off[3];                                                                           // 데이터 수집 시점에 적용되어 있던 센서의 영점 보정(Calibration) 오프셋 값
	float band_energy[T2_Def::Shared::FeatureLimit::BAND_RMS_MAX];                              // 특정 결함 주파수(예: 볼 베어링 외륜 결함 BPFO) 대역에 집중된 에너지 밀도 총합 배열
	float _pad[3];                                                                              // (132B + 12B = 144B) SIMD 128-bit 벡터 연산을 위해 16의 배수로 맞추는 12바이트 패딩
};

// [Block 3] 소음 특화 지표 (240 Bytes)
struct ST_Slot_Audio {
	float		 rms;                                                                           // 소음 전체 대역의 평균 에너지 밀도 레벨
	float		 energy;                                                                        // 소음 파형의 절대 파워 스펙트럼 총합
	float		 centroid;                                                                      // 소음 주파수의 무게 중심
	float		 kurt;                                                                          // 소음 파형 첨도
	float		 crest;                                                                         // 소음 파고율
	float		 sta_lta;                                                                       // 배경 소음 대비 급격히 돌출된 이상 소음 감지 지표
	float		 skew;                                                                          // 파형 왜도
	float		 coh;                                                                           // 공간 결함성 (Stereo Similarity)
	float		 ipd;                                                                           // 채널 간 위상차
	float		 d_rms;                                                                         // RMS 변화율
	...
	float		 band_energy[T2_Def::Shared::FeatureLimit::BAND_RMS_MAX];                       // 밴드별 에너지 밀도
	SpectralPeak top_peaks[T2_Def::Shared::FeatureLimit::TOP_PEAKS_MAX];                        // 상위 피크 정보
	float		 _pad[3];                                                                       // (228B + 12B = 240B) 전체 800B 정합성을 위한 수학적 패딩 조정
};

// [Block 4] AI 텐서 (384 Bytes)
struct ST_Slot_Tensor {
	alignas(16) float mfcc[T2_Def::Audio::FeatureLimit::MFCC_DIM_MAX];                          // 음성 및 기계 소음의 비선형적 인지 특성을 추출한 MFCC 특징량 1D 평탄화 배열 (AI 모델 추론의 핵심 입력 벡터)
};
...
struct PktTelemetry {
	...
	// --- Vib Flattening (AoS 객체 배열 구조를 JS 파싱에 유리한 SoA 평탄화 구조로 전면 해체) ---
	float	 vib_rms[3];                                                                        // X, Y, Z 축별 진동 RMS 평탄화 배열
	float	 vib_centroid[3];                                                                   // X, Y, Z 축별 진동 주파수 무게중심 평탄화 배열
	float	 vib_kurtosis;                                                                      // 진동 첨도 지표
	float	 vib_crest_factor;                                                                  // 진동 파고율 지표
	float	 vib_sta_lta_ratio;                                                                 // 진동 급변 감지 지표 (STA/LTA)
	float	 vib_band_energy[T2_Def::Shared::FeatureLimit::BAND_RMS_MAX];                       // [불변성 보장] 진동 감시 밴드별 에너지 (실 사용 개수와 무관하게 MAX 고정 할당하여 패킷 크기 고정)

	// --- Audio Flattening ---
	float	 audio_rms;                                                                         // 소음 RMS 레벨
	float	 audio_energy;                                                                      // 소음 절대 스펙트럼 에너지
	float	 audio_kurtosis;                                                                    // 소음 파형 첨도
	float	 audio_crest_factor;                                                                // 소음 파형 파고율
	float	 audio_sta_lta_ratio;                                                               // 소음 급변 감지 지표 (STA/LTA)
	float	 audio_skewness;                                                                    // 소음 비대칭성 지표 (왜도)
	float	 audio_spectral_centroid;                                                           // 소음 주파수 무게중심
	float	 audio_band_energy[T2_Def::Shared::FeatureLimit::BAND_RMS_MAX];                     // 소음 감시 밴드별 에너지 (MAX 고정 할당)
	float	 audio_peak_freqs[T2_Def::Shared::FeatureLimit::TOP_PEAKS_MAX];                     // 추출된 상위 피크들의 주파수(Hz) 평탄화 배열
	float	 audio_peak_amps[T2_Def::Shared::FeatureLimit::TOP_PEAKS_MAX];                      // 추출된 상위 피크들의 진폭(Amp) 평탄화 배열
	float	 audio_cpsr_max[T2_Def::Shared::FeatureLimit::CEPS_TARGET_MAX];                     // 타겟 회전체 켑스트럼 피크 최대 진폭 배열
	float	 audio_cpsr_mxrms[T2_Def::Shared::FeatureLimit::CEPS_TARGET_MAX];                   // 전체 대비 타겟 켑스트럼 피크 비율 배열

	// --- AI Tensor ---
	float	 mfcc[T2_Def::Audio::FeatureLimit::MFCC_DIM_MAX];                                   // 머신러닝 모델 추론을 위한 MFCC 1D 평탄화 텐서 배열 (MAX 고정 할당)
};
