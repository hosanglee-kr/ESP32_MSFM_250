...
private:
    ...
    // [넌블로킹 개정] 진동 FIFO 무결성 누적을 위한 X/Y/Z 순환 어큐뮬레이터 링버퍼
    float     _accumX[T2_Def::Vib::Sensor::FFT_SIZE_MAX];
    float     _accumY[T2_Def::Vib::Sensor::FFT_SIZE_MAX];
    float     _accumZ[T2_Def::Vib::Sensor::FFT_SIZE_MAX];
...
public:
    ...
    // [넌블로킹 개정] 진동 FIFO 백그라운드 어큐뮬레이션 및 데이터 인출 API
    uint16_t accumulateVib();
    uint16_t getAccumulatedVib(float* p_outX, float* p_outY, float* p_outZ, uint16_t p_reqCount);
