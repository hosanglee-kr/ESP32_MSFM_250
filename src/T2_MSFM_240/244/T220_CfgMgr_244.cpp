...
// 부동소수점 배열 전용 핫스왑 헬퍼 (배열/단일형 멀티 대응)
...
static inline void cfgSetArray(JsonVariantConst p_v, T (&p_dest)[N]) {
    ...
    if (p_v.is<JsonArrayConst>()) {
        ...
        // 단일 값이 들어온 경우 전체 축/채널에 복제 분배 (하위 호환 보존)
        T v_val = p_v.as<T>();
...
void CL_T2_ConfigManager::_loadDefaults() {
	...
	// [3축 개정] 진동 3축별 독립 임계값 초기화
	for (int i = 0; i < 3; i++) {
		_config.vib_trig.rms_thresh[i]      = T2_Def::Vib::Trigger::RMS_THRESH_DEF;
		_config.vib_trig.kurt_ng_thresh[i]  = T2_Def::Vib::Trigger::KURT_NG_THRESH_DEF;
		_config.vib_trig.crest_ng_thresh[i] = T2_Def::Vib::Trigger::CREST_NG_THRESH_DEF;
		_config.vib_trig.skew_ng_thresh[i]  = T2_Def::Vib::Trigger::SKEW_NG_THRESH_DEF;
	}
	...
	for (uint8_t i = 0; i < T2_Def::Shared::FeatureLimit::BAND_RMS_MAX; i++) {
		...
		// [3축 개정] 3축별 밴드 임계치 2차원 초기화
		for (int a = 0; a < 3; a++) {
			_config.vib_trig.band_thresh[a][i] = 1.0f;
		}
	}
	...
	// [듀얼 개정] L/R 듀얼 채널 독립 임계값 초기화
	for (int ch = 0; ch < 2; ch++) {
		_config.aud_trig.rms_thresh[ch]      = T2_Def::Audio::Trigger::RMS_THRESH_DEF;
		_config.aud_trig.kurt_ng_thresh[ch]  = T2_Def::Audio::Trigger::KURT_NG_THRESH_DEF;
		_config.aud_trig.crest_ng_thresh[ch] = T2_Def::Audio::Trigger::CREST_FACTOR_NG_DEF;
		_config.aud_trig.skew_ng_thresh[ch]  = T2_Def::Audio::Trigger::SKEWNESS_NG_DEF;
	}
	...
	for (uint8_t i = 0; i < T2_Def::Shared::FeatureLimit::BAND_RMS_MAX; i++) {
		...
		// [듀얼 개정] L/R 듀얼 채널 대역 에너지 초기화
		for (int ch = 0; ch < 2; ch++) {
			_config.aud_trig.band_thresh[ch][i] = 1.0f;
		}
	}
	...
	// [듀얼 개정] L/R 양 채널의 센터 FIR 탭을 1.0f(투과)로 초기화
	_config.aud_calib.eq_coeffs[0][T2_Def::Shared::Feature::FIR_TAPS_DEF / 2] = 1.0f;
	_config.aud_calib.eq_coeffs[1][T2_Def::Shared::Feature::FIR_TAPS_DEF / 2] = 1.0f;
...
void CL_T2_ConfigManager::_applyJson(const JsonDocument& p_doc) {
    ...
    if (!v_vt.isNull()) {
        ...
        // [3축 개정] cfgSetArray 헬퍼를 활용한 축별 임계치 파싱 (배열/단일형 겸용)
        cfgSetArray(v_vt["rms_thresh"],     _config.vib_trig.rms_thresh);
        cfgSetArray(v_vt["kurt_ng_thresh"],  _config.vib_trig.kurt_ng_thresh);
        cfgSetArray(v_vt["crest_ng_thresh"], _config.vib_trig.crest_ng_thresh);
        cfgSetArray(v_vt["skew_ng_thresh"],  _config.vib_trig.skew_ng_thresh);
                ...
                if (!v_b["thresh"].isNull()) {
                    if (v_b["thresh"].is<JsonArrayConst>()) {
                        ...
                        for (size_t a = 0; a < 3; a++) {
                            if (a < v_arr.size()) _config.vib_trig.band_thresh[a][i] = v_arr[a].as<float>();
                        }
                    } else {
                        float v_val = v_b["thresh"].as<float>();
                        for (size_t a = 0; a < 3; a++) _config.vib_trig.band_thresh[a][i] = v_val;
                    }
                }
                i++;
            }
    ...
    if (!v_at.isNull()) {
        // [듀얼 개정] L/R 듀얼 채널 독립 임계값 파싱
        cfgSetArray(v_at["rms_thresh"],     _config.aud_trig.rms_thresh);
        cfgSetArray(v_at["kurt_ng_thresh"],  _config.aud_trig.kurt_ng_thresh);
        cfgSetArray(v_at["crest_ng_thresh"], _config.aud_trig.crest_ng_thresh);
        cfgSetArray(v_at["skew_ng_thresh"],  _config.aud_trig.skew_ng_thresh);
        ...
        if (!v_bands.isNull()) {
            ...
            for (JsonObjectConst v_b : v_bands) {
                ...
                // [듀얼 개정] L/R 대역별 임계치 파싱
                if (!v_b["thresh"].isNull()) {
                    if (v_b["thresh"].is<JsonArrayConst>()) {
                        JsonArrayConst v_arr = v_b["thresh"].as<JsonArrayConst>();
                        for (size_t ch = 0; ch < 2; ch++) {
                            if (ch < v_arr.size()) _config.aud_trig.band_thresh[ch][i] = v_arr[ch].as<float>();
                        }
                    } else {
                        float v_val = v_b["thresh"].as<float>();
                        for (size_t ch = 0; ch < 2; ch++) _config.aud_trig.band_thresh[ch][i] = v_val;
                    }
                }
                i++;
            }
            ...
            if (v_eq[0].is<JsonArrayConst>()) {
                ...
                for (size_t ch = 0; ch < 2; ch++) {
                    ...
                    for (JsonVariantConst v_val : v_chEq) {
                        ...
                        _config.aud_calib.eq_coeffs[ch][i] = v_val.as<float>();
                        i++;
                    }
                }
            } else {
                // [하위 호환] 1차원 단일 EQ 파싱 시 L/R 동시 복제 적용
                size_t i = 0;
                for (JsonVariantConst v_val : v_eq) {
                    if (i >= T2_Def::Shared::FeatureLimit::FIR_TAPS_MAX) break;
                    float v_fVal = v_val.as<float>();
                    _config.aud_calib.eq_coeffs[0][i] = v_fVal;
                    _config.aud_calib.eq_coeffs[1][i] = v_fVal;
                    i++;
                }
            }
        }
...
bool CL_T2_ConfigManager::save() {
    ...
    // [3축 개정] 축별 rms 및 통계 NG 임계값 JsonArray 직렬화
    JsonArray v_rmsThV = v_vt["rms_thresh"].to<JsonArray>();
    for(int a=0; a<3; a++) cfgAddFloatExact(v_rmsThV, _config.vib_trig.rms_thresh[a]);
    JsonArray v_kurtThV = v_vt["kurt_ng_thresh"].to<JsonArray>();
    for(int a=0; a<3; a++) cfgAddFloatExact(v_kurtThV, _config.vib_trig.kurt_ng_thresh[a]);
    JsonArray v_crestThV = v_vt["crest_ng_thresh"].to<JsonArray>();
    for(int a=0; a<3; a++) cfgAddFloatExact(v_crestThV, _config.vib_trig.crest_ng_thresh[a]);
    JsonArray v_skewThV = v_vt["skew_ng_thresh"].to<JsonArray>();
    for(int a=0; a<3; a++) cfgAddFloatExact(v_skewThV, _config.vib_trig.skew_ng_thresh[a]);
    ...
    for (uint8_t i = 0; i < _config.vib_trig.active_band_count; i++) {
        ...
        // [3축 개정] 밴드별 3축 임계값 직렬화
        JsonArray v_thArr = v_b["thresh"].to<JsonArray>();
        for(int a=0; a<3; a++) cfgAddFloatExact(v_thArr, _config.vib_trig.band_thresh[a][i]);
    }
    ...
    // [듀얼 개정] L/R 듀얼 채널 독립 임계값 직렬화
    JsonArray v_rmsThA = v_at["rms_thresh"].to<JsonArray>();
    for(int ch=0; ch<2; ch++) cfgAddFloatExact(v_rmsThA, _config.aud_trig.rms_thresh[ch]);
    JsonArray v_kurtThA = v_at["kurt_ng_thresh"].to<JsonArray>();
    for(int ch=0; ch<2; ch++) cfgAddFloatExact(v_kurtThA, _config.aud_trig.kurt_ng_thresh[ch]);
    JsonArray v_crestThA = v_at["crest_ng_thresh"].to<JsonArray>();
    for(int ch=0; ch<2; ch++) cfgAddFloatExact(v_crestThA, _config.aud_trig.crest_ng_thresh[ch]);
    JsonArray v_skewThA = v_at["skew_ng_thresh"].to<JsonArray>();
    for(int ch=0; ch<2; ch++) cfgAddFloatExact(v_skewThA, _config.aud_trig.skew_ng_thresh[ch]);
    ...
    for (uint8_t i = 0; i < _config.aud_trig.active_band_count; i++) {
        ...
        // [듀얼 개정] L/R 듀얼 대역별 임계치 직렬화
        JsonArray v_thArr = v_b["thresh"].to<JsonArray>();
        for(int ch=0; ch<2; ch++) cfgAddFloatExact(v_thArr, _config.aud_trig.band_thresh[ch][i]);
    }
    ...
    // [듀얼 개정] 2차원 스테레오 eq_coeffs 직렬화
    JsonArray v_eq = v_ac["eq_coeffs"].to<JsonArray>();
    for(size_t ch = 0; ch < 2; ch++) {
        JsonArray v_chArr = v_eq.add<JsonArray>();
        for(uint16_t i = 0; i < T2_Def::Shared::FeatureLimit::FIR_TAPS_MAX; i++) {
            cfgAddFloatExact(v_chArr, _config.aud_calib.eq_coeffs[ch][i]);
        }
    }
    ...
    // [안전 기제] 원자적 쓰기 (v244 독립 tmp 파일 격리 스냅숏)
    File v_file = LittleFS.open("/sys/config_244.tmp", "w");
    if (!v_file) {
    ...
    LittleFS.rename("/sys/config_244.tmp", T2_Def::Global::Path::FILE_CFG_JSON_CONST);
