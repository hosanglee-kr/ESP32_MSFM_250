...
private:
    ...
    // v243/244 보완: 5채널(Vib X/Y/Z 3축 + Audio L/R 2채널) 히스토리 독립 관리 (PSRAM 할당 권장)
...
public:
    ...
    /**
     * @brief [듀얼 개정] 소음 특징량 추출 (L/R 개별 RMS, Spectral, 켑스트럼, MFCC39 특징 추출)
     */
    void extractAudio(const float* p_audL, const float* p_audR, uint32_t p_len, uint32_t p_sampleRate, T2_Type::UnifiedFeatureSlot& p_slot, const T2_Type::ST_Audio_Feature& p_featCfg, const T2_Type::ST_Audio_Trigger& p_trigCfg);
