...
// [넌블로킹 개정] 진동 FIFO 백그라운드 누적 링버퍼 삽입 함수
uint16_t CL_T2_SensorEngine::accumulateVib() {
    if (!_isBmiInit || _isPaused) return 0;

    // FIFO에서 안전하게 한 배치 읽어옴
    alignas(16) float v_tempX[T2_Def::Vib::Hardware::FIFO_BATCH_SIZE_MAX];
    alignas(16) float v_tempY[T2_Def::Vib::Hardware::FIFO_BATCH_SIZE_MAX];
    alignas(16) float v_tempZ[T2_Def::Vib::Hardware::FIFO_BATCH_SIZE_MAX];

    uint16_t v_read = readVibFifoBatch(v_tempX, v_tempY, v_tempZ, T2_Def::Vib::Hardware::FIFO_BATCH_SIZE_MAX);
    if (v_read == 0) return 0;

    // 링버퍼에 데이터 적재 (오버플로우 시 덮어쓰기)
    const uint16_t BUFFER_SIZE = T2_Def::Vib::Sensor::FFT_SIZE_MAX;
    for (uint16_t i = 0; i < v_read; i++) {
        _accumX[_accumWriteIdx] = v_tempX[i];
        _accumY[_accumWriteIdx] = v_tempY[i];
        _accumZ[_accumWriteIdx] = v_tempZ[i];

        _accumWriteIdx = (_accumWriteIdx + 1) % BUFFER_SIZE;

        if (_accumCount < BUFFER_SIZE) {
            _accumCount++;
        } else {
            // 오버플로우: 가장 오래된 프레임 포기하고 리드 인덱스 강제 이동
...
// [넌블로킹 개정] 누적된 진동 데이터를 요청 크기만큼 꺼내오는 인출 함수
uint16_t CL_T2_SensorEngine::getAccumulatedVib(float* p_outX, float* p_outY, float* p_outZ, uint16_t p_reqCount) {
