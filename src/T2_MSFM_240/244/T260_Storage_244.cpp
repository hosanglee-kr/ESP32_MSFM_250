...
bool CL_T2_StorageManager::_saveIndexAtomic() {
    File v_tmp = LittleFS.open("/sys/index_244.tmp", "w");
    ...
    LittleFS.remove("/sys/runtime_idx_244.json");
    LittleFS.rename("/sys/index_244.tmp", "/sys/runtime_idx_244.json");
    return true;
}

bool CL_T2_StorageManager::_loadIndex() {
    if (!LittleFS.exists("/sys/runtime_idx_244.json")) return false;

    File v_file = LittleFS.open("/sys/runtime_idx_244.json", "r");
...
bool CL_T2_StorageManager::attemptRecovery() {
    ...
    ESP_LOGW(TAG, "Attempting SD Card Auto-Recovery (v244)...");
    SD_MMC.end();
