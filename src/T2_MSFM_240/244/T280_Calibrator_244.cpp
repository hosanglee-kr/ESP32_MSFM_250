/* ============================================================================
 * File: T280_Calibrator_244.hpp
 * Summary: v243 멀티모달 캘리브레이션 엔진 구현부
 * ========================================================================== */
#include "T280_Calibrator_244.hpp"
...
void CL_T2_Calibrator::_processAuto(const char* p_path) {
    ESP_LOGI(TAG, "Starting Auto Noise Profiling (v244): %s", p_path);
    ...
    while (v_file.available() >= sizeof(T2_Type::UnifiedRawChunk)) {
        ...
        for (int i = 0; i < T2_Def::Audio::Sensor::FFT_SIZE_MAX; i++) {
            v_sumSqAudioL += (double)(v_chunk->audio_l[i] * v_chunk->audio_l[i]);
            v_sumSqAudioR += (double)(v_chunk->audio_r[i] * v_chunk->audio_r[i]);
        }
        v_totalChunks++;
        esp_task_wdt_reset();
    }
    ...
    float v_baseAudL = (float)sqrt(v_sumSqAudioL / (v_totalChunks * T2_Def::Audio::Sensor::FFT_SIZE_MAX));
    float v_baseAudR = (float)sqrt(v_sumSqAudioR / (v_totalChunks * T2_Def::Audio::Sensor::FFT_SIZE_MAX));
    float v_maxBaseAud = fmaxf(v_baseAudL, v_baseAudR);

    // v244 4-Tier 설정 반영 (노이즈의 3배를 임계치로 자동 설정)
    T2_Type::DynamicConfig v_cfg = CL_T2_ConfigManager::getInstance().getConfig();
    for (int a = 0; a < 3; a++) {
        v_cfg.vib_trig.rms_thresh[a] = fmaxf(v_baseVib * 3.0f, 0.01f);
    }
    for (int ch = 0; ch < 2; ch++) {
        v_cfg.aud_trig.rms_thresh[ch] = fmaxf(v_maxBaseAud * 3.0f, 0.005f);
    }
    ...
    if (v_csv) {
        v_csv.printf("%llu,%.6f,%.6f,%.6f\n", (uint64_t)time(NULL), v_baseVib, v_baseAudL, v_baseAudR);
        v_csv.close();
    }

    SD_MMC.remove(p_path);
    ESP_LOGI(TAG, "Auto Calib Completed. Vib:%.5f, AudL:%.5f, AudR:%.5f", v_baseVib, v_baseAudL, v_baseAudR);
}

void CL_T2_Calibrator::_processManual(const char* p_path) {
    ESP_LOGI(TAG, "Starting Advanced FIR-Inverse Calib (v244): %s", p_path);
    ...
    while (v_file.available() >= sizeof(T2_Type::UnifiedRawChunk)) {
        ...
        // [듀얼 개정] Left/Right 파형을 산술 평균하여 물리 편차 보완한 기준 파형 생성
        for (uint32_t i = 0; i < v_samples; i++) {
            v_fftWork[i * 2] = (v_chunk->audio_l[i] + v_chunk->audio_r[i]) * 0.5f;
    ...
    if (v_count > 0) {
        ...
        for (int i = 0; i < v_taps; i++) {
            ...
            // [듀얼 개정] 계산된 동일 역필터 계수를 L/R 채널 모두에 동일하게 복사 적용
            v_cfg.aud_calib.eq_coeffs[0][i] = v_val;
            v_cfg.aud_calib.eq_coeffs[1][i] = v_val;
        ...
        if (v_sumAbs > v_cfg.aud_calib.norm_safe) {
            ...
            for (int i = 0; i < v_taps; i++) {
                v_cfg.aud_calib.eq_coeffs[0][i] *= v_scale;
                v_cfg.aud_calib.eq_coeffs[1][i] *= v_scale;
            }
            ESP_LOGW(TAG, "FIR Normalized by L1-Norm (v244): %.3f", v_scale);
        }
