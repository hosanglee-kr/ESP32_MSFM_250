...
public:
    ...
    struct ST_AudioDspRuntime {
        ...
        float             prev_pre_emp[2];                                                      // [듀얼 개정] L/R 개별 프리엠파시스 샘플
        alignas(16) float median_hist[2][T2_Def::Shared::Dsp::MEDIAN_WINDOW_MAX];               // [듀얼 개정] L/R 개별 메디안 필터 히스토리
        alignas(16) float notch_coeffs[5];
        alignas(16) float notch_state[2][2];                                                   // [듀얼 개정] L/R 개별 Notch 상태 (notch_state[L/R][order])
        alignas(16) float notch2_coeffs[5];
        alignas(16) float notch2_state[2][2];                                                  // [듀얼 개정] L/R 개별 Notch2 상태
        alignas(16) float iir_hpf_coeffs[5];
        alignas(16) float iir_hpf_state[2][2];                                                 // [듀얼 개정] L/R 개별 IIR HPF 상태
        alignas(16) float iir_lpf_coeffs[5];
        alignas(16) float iir_lpf_state[2][2];                                                 // [듀얼 개정] L/R 개별 IIR LPF 상태
        fir_f32_t         fir_inst_hpf[2];                                                      // [듀얼 개정] L/R 개별 FIR HPF 인스턴스
        fir_f32_t         fir_inst_lpf[2];                                                      // [듀얼 개정] L/R 개별 FIR LPF 인스턴스
        ...
        alignas(16) float fir_state_hpf[2][T2_Def::Shared::FeatureLimit::FIR_TAPS_MAX];         // [듀얼 개정] L/R 개별 FIR HPF 상태 버퍼
        alignas(16) float fir_state_lpf[2][T2_Def::Shared::FeatureLimit::FIR_TAPS_MAX];         // [듀얼 개정] L/R 개별 FIR LPF 상태 버퍼
    };
    ...
    struct ST_AccelDspRuntime {
        ...
        alignas(16) float fir_hpf_coeffs[T2_Def::Vib::Sensor::AXIS_MAX][T2_Def::Shared::FeatureLimit::FIR_TAPS_MAX]; // [3축 개정] X, Y, Z 개별 HPF 계수
        alignas(16) float fir_lpf_coeffs[T2_Def::Vib::Sensor::AXIS_MAX][T2_Def::Shared::FeatureLimit::FIR_TAPS_MAX]; // [3축 개정] X, Y, Z 개별 LPF 계수
...
public:
     ...
     * [듀얼 개정] Beamforming 활성 여부에 따른 스테레오 개별 독립 필터링 파이프라인 작동
     */
