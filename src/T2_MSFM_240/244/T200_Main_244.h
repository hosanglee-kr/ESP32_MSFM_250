/* ============================================================================
 * File: T200_Main_243.h
 * Summary: v243 4-Tier 시스템 통합 진입점 및 전역 관리 (main.cpp 연동용)
 * ============================================================================
 * [로직 상태 추적 (Logic State Tracker)]
 * - 유지: esp_timer 기반 ISR 디바운싱 및 핵심 부팅 시퀀스.
 * - 갱신: v243 4-Tier 설정 체계에 따른 하드웨어 핀 맵핑 적용.
 ...
 */
void IRAM_ATTR T243_handleTriggerISR() {
    ...
    if (v_now_us - v_last_us > 200000ULL) {
        ...
        if (v_active) v_fsm_ref.dispatchCommand(T2_Type::SystemCommand::CMD_START);
        else v_fsm_ref.dispatchCommand(T2_Type::SystemCommand::CMD_STOP);
    }
}
...
inline void T2_init() {
    ...
    Serial.println("\n[SMEA-T240] v243 4-Tier Diagnostic System Booting...");

    // 2. 하드웨어 핀 설정 및 초기 인터럽트 동기화
    pinMode(T2_Def::Global::Hardware::PIN_BTN_CONTROL_CONST, INPUT_PULLDOWN);
    attachInterrupt(digitalPinToInterrupt(T2_Def::Global::Hardware::PIN_BTN_CONTROL_CONST), T243_handleTriggerISR, CHANGE);
