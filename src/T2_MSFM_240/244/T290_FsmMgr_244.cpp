...
bool CL_T2_FsmManager::init() {
    ESP_LOGI(TAG, "Initializing v244 Orchestrator...");
    ...
    // [듀얼 개정] L/R 양방향 채널의 완벽 독립 가속을 위해 R 채널 고속 버퍼도 항시 동시 할당
...
void CL_T2_FsmManager::_processTask(void* p_param) {
    ...
    while (1) {
        if (xQueueReceive(v_this->_qReadyIdx, &v_idx, pdMS_TO_TICKS(10))) {
            ...
            // [v015 이식] PSRAM 데이터를 Internal SRAM 고속 버퍼로 복사하여 연산 가속 (Left / Right 모두 복사)
            if (v_cfg.system.audio_enable) {
                ...
                if (v_this->_prcBufR && v_raw.audio_r) {
            ...
            if (v_cfg.system.audio_enable) {
                // [듀얼 개정] L/R 고속 버퍼에 대해 인플레이스 필터링 구동 (_prcBufL/R을 입력이자 동시에 출력으로 재활용)
                v_this->_dsp.processAudio(v_this->_prcBufL, v_this->_prcBufR, v_this->_prcBufL, v_this->_prcBufR, T2_Def::Audio::Sensor::FFT_SIZE_MAX, v_cfg.shared_dsp, v_cfg.aud_dsp);
            }
            ...
            if (v_cfg.system.audio_enable) {
                // [듀얼 개정] L/R 양방향 채널의 특징량을 병렬로 추출하고, 교차 스펙트럼 위상 분석 수행
                v_this->_extractor.extractAudio(v_this->_prcBufL, v_this->_prcBufR, T2_Def::Audio::Sensor::FFT_SIZE_MAX, v_cfg.aud_sensor.sample_rate, v_slot, v_cfg.aud_feat, v_cfg.aud_trig);
            }

            // [Step 3] 시퀀스 빌딩 (ML용) - MFCC 텐서의 5채널(Vib 3 + Audio 2) 대규모 특징 축적 대응
            if (v_cfg.system.audio_enable) {
...
void CL_T2_FsmManager::_broadcastStreams(const T2_Type::UnifiedFeatureSlot& p_slot, const T2_Type::UnifiedRawChunk& p_raw) {
    ...
    // 1. Telemetry (상태 및 특징량 요약 - [3축 / 듀얼 채널 개정])
    ...
    if (_accTele >= 100) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            _pktTele->vib_kurtosis[i] = p_slot.vib.kurt[i];
            _pktTele->vib_crest_factor[i] = p_slot.vib.crest[i];
            _pktTele->vib_skewness[i] = p_slot.vib.skew[i];
            _pktTele->vib_std[i] = p_slot.vib.std[i];
            memcpy(_pktTele->vib_band_energy[i], p_slot.vib.band_energy[i], sizeof(_pktTele->vib_band_energy[i]));
        }
        ...
        for (int ch = 0; ch < 2; ch++) {
            ...
            // [듀얼 개정] 오디오 채널은 표준편차(std) 지표 미지원이므로 제거
            ...
            for (int p = 0; p < T2_Def::Shared::FeatureLimit::TOP_PEAKS_MAX; p++) {
            ...
            for (int c = 0; c < T2_Def::Shared::FeatureLimit::CEPS_TARGET_MAX; c++) {
        ...
        _pktTele->audio_ts = p_slot.header.ts; // 물리 오디오 동기 타임스탬프
        _pktTele->vib_ts = p_slot.header.ts;   // 물리 진동 동기 타임스탬프
    ...
    // 2. Waveform (오디오 + 진동 순차 전송 - v234 정합성 및 듀얼 소음 확장)
    ...
    if (_accWave >= 100) {
        ...
        _pktWave->header.source = 1; // Audio Left
        ...
        // Audio R (Source ID = 5) - STEREO 모드 구동 시
        if (v_cfg.aud_sensor.channel_mode == T2_Type::AudioChannelMode::STEREO) {
            _pktWave->header.magic = 0xAA;
            _pktWave->header.type = (uint8_t)T2_Type::StreamType::WAVEFORM;
            _pktWave->header.source = 5; // Audio Right
            _pktWave->header.len  = T2_Def::Audio::Sensor::FFT_SIZE_MAX * sizeof(float);
            memcpy(_pktWave->samples, p_raw.audio_r, _pktWave->header.len);
            _comm.broadcastBinary(_pktWave, sizeof(T2_Type::PktWaveform));
        }

        // Vibration (X, Y, Z 순차 전송 - Source ID = 2, 3, 4)
        for (int i = 0; i < 3; i++) {
