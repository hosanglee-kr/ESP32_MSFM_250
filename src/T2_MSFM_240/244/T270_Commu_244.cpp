 ...
 * Summary: 멀티플렉싱 통신 엔진 구현부 (v243)
 * ========================================================================== */
#include "T270_Commu_244.hpp"
...
static const char* TAG = "T243_COMM";
...
bool CL_T2_Communicator::init() {
        ...
        for (uint8_t i = 0; i < T2_Def::Global::NetLimit::MAX_MULTI_AP_CONST; i++) {
            if (strlen(v_cfg.wifi.multi_ssid[i]) > 0) {
                ...
                if (WiFi.status() == WL_CONNECTED) {
                    ...
                    // NTP 서버 동기화
                    configTzTime(T2_Def::Global::Net::TZ_INFO_CONST, T2_Def::Global::Net::NTP_SERVER_1_CONST, T2_Def::Global::Net::NTP_SERVER_2_CONST);
                    _enforceTcpKeepAlive(_wifiClient.fd()); // [v243 복원] 좀비 소켓 방어
                    v_connected = true;
                    break;
                }
            }
    ...
    ESP_LOGI(TAG, "Communication Engine v243 Ready.");
    return true;
}
...
void CL_T2_Communicator::_initWebHandlers() {
    ...
    _server.on("/api/command", HTTP_POST, [](AsyncWebServerRequest* p_req) {
        ...
        T240_DispatchCommand((T2_Type::SystemCommand)v_cmd);
    ...
    // [v243 복원] 파일 브라우징 및 다운로드
    _server.on("/api/files", HTTP_GET, [](AsyncWebServerRequest* p_req) {
        if (LittleFS.exists("/sys/runtime_idx_244.json")) {
            p_req->send(LittleFS, "/sys/runtime_idx_244.json", "application/json");
        } else {
    ...
    _server.on("/api/update", HTTP_POST, [](AsyncWebServerRequest *p_req) {
        ...
        T240_DispatchCommand(T2_Type::SystemCommand::CMD_OTA_END);
        if (!Update.hasError()) {
        ...
        if (p_index == 0) {
            ...
            // [v015 이식] OTA 시작 시 시스템 버스(SPI/SD) 충돌 방어
            T240_DispatchCommand(T2_Type::SystemCommand::CMD_OTA_START);
...
bool CL_T2_Communicator::publishResultMqtt(const T2_Type::UnifiedFeatureSlot& p_slot, T2_Type::DetectionResult p_result) {
    if (!_mqttHandle || p_result == T2_Type::DetectionResult::PASS) return false;
    ...
    // [3축 개정] 진동 X/Y/Z 개별 RMS 전송
    JsonArray v_rmsV = v_doc["rms_v"].to<JsonArray>();
    v_rmsV.add(p_slot.vib.rms[0]);
    v_rmsV.add(p_slot.vib.rms[1]);
    v_rmsV.add(p_slot.vib.rms[2]);

    // [듀얼 개정] 소음 Left/Right 개별 RMS 전송
    JsonArray v_rmsA = v_doc["rms_a"].to<JsonArray>();
    v_rmsA.add(p_slot.audio.ch[0].rms);
    v_rmsA.add(p_slot.audio.ch[1].rms);
    
    // 공간 결함성 지표(coh, ipd) 전송
