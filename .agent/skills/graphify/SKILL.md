...
## Customizing and Updating Wiki & CodeGraph (Project-Specific Refinement)

For complex projects, you can refine the knowledge graph and clean up unwanted records from CodeGraph database using a configuration file and helper python scripts.

### 1. Configuration: `graphify_config.json`
By default, the configuration file is placed in the **Skill Folder** (`.agent/skills/graphify/graphify_config.json`). It can also be overridden per project by placing a file with the same name at the workspace root directory.

#### Configuration Schema:
- **`workspace_dir`**: The path to project root (usually `"."`).
- **`wiki_out_dir`**: Output directory for generated wiki pages (e.g., `src/T2_MSFM_250_wiki`).
- **`components`**: Map of component names to their physical directories.
- **`filters`**: Define allowed file extensions per component. If defined, files not matching the extension rules will be skipped from the wiki/graph.
- **`folders_to_clean`**: Subfolders (like individual `graphify-out` directories) to automatically clean up after a successful build.
- **`codegraph`**:
  - `db_path`: Database path to SQLite db (default `.codegraph/codegraph.db`).
  - `remove_patterns`: Array of patterns for removing files/nodes from the CodeGraph database. Supports:
    - `prefix`: Absolute/relative directory path prefix.
    - `exclude_extensions`: Keep only these extensions in the prefix directory; remove everything else.
    - `contains`: Remove any files whose path contains this string.

---

### 2. Helper Scripts
These scripts are located under `.agent/skills/graphify/scripts/`. 

*Note: You do not need to manually configure python virtual environments. The scripts will automatically bootstrap and install necessary packages (`graphifyy`, `networkx`) under `.agent/skills/graphify/.venv/` on their first run.*

#### A. Wiki & Obsidian Update Pipeline (`update_wiki.py`)
Re-builds a curated, filtered knowledge graph based on a pre-existing `.graphify_extract.json` file. It maps old community Korean labels, runs LLM fallback for missing ones, cleans up individual subfolders, and generates final Obsidian, Canvas, Wiki, and HTML outputs.
...
- **Execution**:
  ...
  ```
- **CLI Options**:
  - `--config <path>`: Custom configuration path (optional).
  - `--python-path <path>`: Deprecated. Now handled automatically by the bootstrap virtualenv.

#### B. CodeGraph Clean Tool (`update_codegraph.py`)
Syncs/indexes the codebase and removes unwanted library directories or temporary files from CodeGraph SQLite DB to prevent visual clutter and keep focus only on relevant symbols.
